...
| browser storage namespace | 上記全項目を実装。ADR-0063 D9によりtrusted auth/session persisterのbackend runtime接続は完了。Playwright E2E (8 tests) によりマルチタブテナント切替・bfcache復元・auth-required・stale 409を実ブラウザで検証済み | 実capability/PDP service連携と完全consumer matrixは Phase 2 |
