...
| --- | --- | --- |
| コンポーネント、信頼境界、責務 | `02_Architecture/architecture.html` | 型名・endpoint名への参照 |
...
| 物理保存、CRUD、support level、運用責任 | `02_Architecture/data_model_operations_overview.html` | schema型名とAPI endpointへの参照 |
...
| --- | --- | --- | --- | --- | --- |
| CI-CE1-01 | `ContextQueryV1` | `02_Architecture/architecture.html`旧§7A.2.1: `scope:string[]`、depth文字列列挙、SafeMode object、`queryId`/`previewConfirmed`なし | `schemas.md` §1.2: `queryId`、scope列挙、depth数値、`safeModePolicy:"strict"`、`previewConfirmed` | model/fixture/contract testは候補比較の証拠。上位契約を置換する根拠にはしない | **Resolved 2026-07-15 by `CE1-CONTRACT-01`**。旧signatureはInformative history、現行logical queryは`schemas.md`の9-key closed-world型 |
...
| CI-DOC-03 | support level | `02_Architecture/data_model_operations_overview.html` §4.1は`cards[].meta`、`contradictionSignalDecisions`、`cards[].ka`を掲載 | `holdState`と`shelf`は`cards[]`説明へ包含され、独立行がない | support levelを変えず、検索可能な独立行が必要か確認する | **Clarified 2026-07-15**。`cards[].holdState` / `shelf`をL2.5独立行として追加 |
...
| --- | --- | --- | --- |
| H-A | `02_Architecture/architecture.html` | **Moved 2026-07-15** to `history/architecture-contract-freeze-formation-2026-04-to-05.md`: §7A.0 snapshot、§7A.2.1 Interface Freeze、§12後の`Contract Freeze Baseline`、§13 Stream Reflection Note | コンポーネント責務、信頼境界、採択済み契約IDへの索引 |
...
| H-D | `02_Architecture/data_model_operations_overview.html` | §1.2/1.3 Stream注記、§8〜§13 execution log/checkpoint/record/sync | **Moved 2026-07-15** to `history/data-model-operations-stream-d-2026-05.md`。現行物理モデル、CRUD、support level、運用責任は元文書に維持 |
...
rg -n "^#{1,4} .*?(Stream|freeze|Freeze|rerun|execution log|checkpoint|reaffirmation|handoff)" 02_Architecture/architecture.html 02_Architecture/api.md 02_Architecture/schemas.md 02_Architecture/data_model_operations_overview.html
```
