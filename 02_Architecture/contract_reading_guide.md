...
6. **エンドポイントを廃止する場合は、`api.md` §13 の機械可読な廃止宣言**（`- 廃止: <METHOD> <path> — <issue>（<日付>、<方向>）`）を置く。これにより `check_design_consistency.py` が「廃止済み」と「未文書化」を区別し、廃止した機能を論じる設計文書へ偽陽性を出さない（DX-CANON-INTENT-01 案A）。**設計正本に実装可能なスキーマを残さない**（廃止は散文の記録と廃止宣言行のみ）。
