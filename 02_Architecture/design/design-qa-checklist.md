...
- 特記（2026-07-09時点）: 「作業」メニューは「作業モード」単一入口とし、当時の実装では積層セクション方式を採用していたため、存在しないタブ構造を偽装しない判断を記録した。「共有」メニューも「共有と再現」単一入口＋SafeModeトグルのみとし、共有前確認・公開範囲・出典参照等はSharePanel側へ委譲した。作業モードについては、その後 `ADR-0055` と `UX-NAV-02` により5タブ方式へ更新されている。
...
## 第8回 2026-07-12: P32 承認事項の反映（teal凡例・sky近接確認）

Claude Design 第2回照合（P32）が teal=検索一致を承認した際の2条件への対応記録。

| 節 | 結果 | 所見 |
| --- | --- | --- |
| 条件1: 凡例へ「一時」枠で分離掲載 | ✓ | `CanvasLegend.tsx` に破線区切りの新グループ「一時的な表示」を追加し、「検索一致（検索中のみ）」を teal 枠スウォッチで掲載。恒久4チャネル（型/保持/確認/根拠）とは視覚的に分離。ja/en 両ロケール追加。 |
| 条件2: evidence=sky との近接判別の実機確認 | ✓（同日解消） | 当初「線が描画されず取得不能」だったが、DOMプローブで**実在バグを発見**: `EvidenceOverlayLayer` が HTML直下root `<svg>` で無視される `x`/`y` 属性＋`inset:0` の旧パターンを使っており、**根拠オーバーレイの線はDOMに存在するが常に不可視**だった（EdgeLayer は同一故障モードを修正済みで、その修正が未移植だった）。EdgeLayer と同じ CSS `left/top` 方式へ修正し、修正後の実機で teal枠（#0d9488 3px 輪郭）と sky実線（#0369a1 2px）の同時描画を取得。**判別可能**（色相族: 緑青 vs 青、形状: 枠 vs 線）。観察事項: 線はカード中心間を結ぶため zIndex 3 でカード面を横切る——終端をカード境界で止めるかは次回照合の設計確認事項として記録のみ。 |
| 参考 | - | 空キャンバスCTAの teal `#0f766e`（第7回で確認済み）はボタンの視覚役割であり、カード状態チャネルの teal=検索一致（一時）とは役割が異なる。凡例には CTA を載せない方針を維持。 |

- 生成物: `03_Implement/frontend/scripts/verify_teal_sky_20260712.mjs`（一時利用）。
...
| 節 | 結果 | 所見 |
| --- | --- | --- |
| A. 視覚言語 | △（設計側予備所見、未検証） | P35: N-1の生絵文字・手描き風グリフは4チャネル1意味（P1）から明確に逸脱と指摘。P37がslate単色線画（塗りなし・1.5px・currentColor=slate-500）＋本文左20×20専用スロットへの再設計案（型色・amberと色相非衝突）を提示。プロトタイプ側にcueフィールドと描画スロットを例示追加済みだが設計側の例示であり本実装ではなく、実機確認は未了。 |
| B. 状態遷移 | △（設計側予備所見、未検証） | N-2はラウンドタイムライン・引き継ぎ/再開ブリーフ・分岐比較を含み保留/違和感/可逆性の核と親和性ありとP35が評価。ただし「探究終了」等の破壊的操作はA-1と同じ3択（保存/破棄/取消）＋SafeMode既定ON継承が必要と指摘、実機未確認。F-1のTenantSessionRuntimeGateは切替確認中の同時表示禁止＋request cache破棄を含む状態リセットがP34追加分としてP36に記載、実機未確認。 |
| C. 核の保護 | △（設計側予備所見、未検証） | P37: N-1の5段階評価は反スコアリングに反するため製品面から全廃、試行時間・誤答数は集計・平均・ランキングを出さない「試行ログ（事実の時系列）」に読み替えと明記。P35: N-2の分岐比較・カード来歴は反スコアリング方針と抵触せず方向自体に反対なしと評価。両者ともCB自己申告（P37はCB-1〜4を明記）はあるが、本チェックリストとしての確定判定ではない。 |
| D. a11y/契約 | △（設計側予備所見、未検証） | F-1のTenantChangeConfirmationDialogは未保存変更ありの3択モーダルとP36に記載。N-2のimport/export・分岐検出は出典/文面のサニタイズ適用対象とP35が指摘。フォーカス順・role/aria等の実機確認は本ラウンドでは未実施。 |

- **本ラウンドの位置づけ: 予備所見の記録のみ、実機照合は未了。** 上表の△はすべて「Claude Design自身が申告した暫定推奨」（P35の明示的な「推奨判定=△（正式化条件つき）」、およびP36/P37の設計方針）を転記したものであり、本リポジトリ側でスクリーンショットを取得し独自に確認した結果ではない。N-1は正式化条件（視覚トークン差し替え＋計測器の反スコアリング化）完了までは実験用限定タブとして隔離が暫定線引き。N-2はP29確定6分類のどこに位置するか実機照合時に判断（新規7分類は作らない＝CB-1）。F-1はADR-0059/0061のImplementation gate成立後、F-2着手前段の確認事項。
- 次回対応: N-1/N-2の主要状態＋「探究終了」確認モーダル＋import/exportの実機スクリーンショット、およびF-1の4コンポーネント（bootstrap/runtime gate blocked state・確認dialog・switcher単一/複数）のスクリーンショットを取得し、P28チェックリスト自己申告とともに正式照合ラウンドを実施予定。
