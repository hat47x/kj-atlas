...
| 条件2: evidence=sky との近接判別の実機確認 | △ 持ち越し | teal枠自体の実機確認は取得（`verify_teal_sky_20260712.mjs`: 通常一致 #5eead4 2px・アクティブ一致 #0d9488 3px、**選択時もアクティブ検索一致のtealが選択青より優先描画されることを確認**）。ただし supports線（#0369a1）との同時描画は、根拠オーバーレイ有効＋対象カード選択のフィクスチャでも線が描画されず取得できなかった。オーバーレイの描画条件（ビュー種別・深さ等）の追加調査のうえ次回照合へ持ち越し。静的判定では teal（緑青）と sky-700（青）は色相族が異なり、線（stroke）と枠（outline）で形状チャネルも異なる。 |
