...
| `KJ_ATLAS_SAAS_OAUTH_BROKER_HTTP_REDIRECT_URI` | 未設定 | OAuth フローの redirect URI（ADR-0074）。path は `/session/callback` 固定 | direct | 通常値（接続先URL。認証情報は別キー） | フロー完了後のリダイレクト先が設定値と一致することを確認する |
