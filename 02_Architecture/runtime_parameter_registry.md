...
| `KJ_ATLAS_APP_REVISION` | 未設定 | OPS-OBSERV-01: 稼働中のビルド識別子。`GET /version` と診断バンドルの `app.revision` に現れる。frontend では build ARG として渡す（`vite.config.ts` の `envPrefix` 経由） | direct / base Compose（web build arg） | 通常値 | `GET /version` の `revision` が設定値を返すこと、診断バンドルが `unknown` でないことを確認する |
