...
代表視覚手掛かりは生成AIだけに依存しません。標準KJ法の手描きシンボルと連続する基本描画・利用者画像、通信不要の絵文字・同梱プリセット、権利確認済み外部素材、proposal-only画像生成を別経路とします。写真・図が観察データである場合は、識別用手掛かりではなく一次視覚資料として元文脈を保持します。意味境界と段階導入は `00_Prompt/representative_visual_cue_requirements.md`、供給・保存の判断は `ADR-0060` を参照します。現行MVPに画像生成APIまたは永続契約が実装済みであることを示すものではありません。
