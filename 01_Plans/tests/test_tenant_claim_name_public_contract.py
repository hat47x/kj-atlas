...
class TenantClaimNamePublicContractTest(unittest.TestCase):
    def test_settings_default_is_not_runtime_fixed_requirement(self) -> None:
        ...
        self.assertIn('SUI_TENANT_CLAIM_NAME must not be empty', settings)
        self.assertIn('SUI_TENANT_CLAIM_NAME must be ≤ 256 characters', settings)
        self.assertIn('SUI_TENANT_CLAIM_NAME must not have leading/trailing whitespace', settings)
        self.assertIn('SUI_TENANT_CLAIM_NAME must not contain spaces', settings)
    ...
    def test_public_surfaces_distinguish_default_from_requirement(self) -> None:
        ...
        self.assertNotIn('`SUI_TENANT_CLAIM_NAME=tenant_ref`', registry)
