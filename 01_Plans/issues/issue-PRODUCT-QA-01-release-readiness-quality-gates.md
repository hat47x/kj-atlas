...
- The P2A readiness panel now localizes user-facing readiness, reason, accepted/blocked, and validation-result labels instead of exposing raw `go`, `pass`, or `fail` values in Japanese UI.
...
- G2 primary operations: Go (updated same day). The named-scope reconciliation succeeded, `QA-MONKEY-13` is now fixed and verified, and the full suite is fully green (165/165).
- G3 Japanese UI: unchanged, reconfirmed Go per prior records; no new locale drift reported in this slice.
- G4 viewport/operability: Go (updated same day). `QA-MONKEY-13` (the keyboard/operability defect) is fixed and verified; physical keyboard and screen-reader acceptance remain outstanding NON-DELEGATED gates (human acceptance testing), independent of and not satisfied by this automated fix -- those are still required before shipment.
- G6 diagnostics/support: unchanged from the 2026-07-11 rehearsal records.
- G7 build/regression: Go (updated same day). E2E moved from 17 known deterministic failures to 0: `QA-MONKEY-13` is fixed and the full suite passes 165/165. Typecheck clean (`tsc --noEmit`, no errors); Vitest **190/190 test files, 1034/1034 tests passing**.

### Blocker Inventory

- `QA-MONKEY-13` (Alt+Shift+2 hierarchy shortcut): **fixed and verified same day** (root cause: a maxDepth-clamp/hierarchyLevel-sync effect feedback loop in `App.tsx`; see this record's Decision for detail). No longer a blocker. Full e2e suite is 165/165.
...
**Conditional Go for the `DX-E2E-07` contract-reconciliation batch, the `ADR-0052`/`UI-QUALITY-A11Y-03` closure, and `QA-MONKEY-13` / No-Go for full release shipment (non-delegated gates only).** `DX-E2E-07` may be closed on its own declared scope (all 17 originally-identified failures classified; 16 fixed as test drift, 1 correctly split into `QA-MONKEY-13` as a product defect). **Update, same day**: `QA-MONKEY-13` has since been root-caused and fixed. Root cause: a view-state depth-clamp `useEffect` (watching `[maxAvailableDepth, maxDepth]`) unconditionally reduced `maxDepth` to `maxAvailableDepth` whenever it was exceeded, even when `maxAvailableDepth === 0` (no islands nested past the top level); this clamp change then re-triggered a second `useEffect` (watching `[maxDepth]`) that re-derives `hierarchyLevel` from `maxDepth`, silently demoting the user's explicit "mid" selection back to "overview" -- a two-effect feedback loop, not a bug in the Alt+Shift+2 key handler itself (confirmed via live instrumentation: `handleHierarchyLevelChange` received and applied `"mid"` correctly; the second effect's cascade overwrote it milliseconds later). Fix: added `&& maxAvailableDepth > 0` to the clamp condition -- when there is no deeper island content to clamp against, `maxDepth=1` and `maxDepth=0` are behaviorally indistinguishable, so clamping only serves to override the user's choice. Full re-verification after the fix: typecheck clean; Vitest 190/190 files, 1034/1034 tests; **full Playwright suite 165/165 passing -- zero failures, the first fully-clean full-suite run this session**. G2/G4/G7 are therefore no longer blocked by any known e2e defect. This record does **not** claim full release readiness: all non-delegated gates from prior closeouts (final program approval, physical keyboard/screen-reader acceptance, release screenshot approval, formal organization approval) remain outstanding, as do the separately-tracked `DATA-MAINT-04` scope-pick and the intentionally-deferred `EXT-CONN-01` subslice C.
