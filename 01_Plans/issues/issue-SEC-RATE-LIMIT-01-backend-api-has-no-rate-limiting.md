...
- **追記（2026-08-12・検証）**: 本文の「`/admin/provision/users`は認証依存なし」は**部分的に古い**。現行は `require_single_tenant_provisioning_surface`（`routes/admin.py:78`）で**単一テナントモードに限定**され、SaaS ランタイムでは 404（曝露縮小）。ただし単一テナントでは未レート制限のまま。JIT provisioning 経路の現行ゲートは個別確認が必要。
