...
- 現在の問題: `domain.md` は用語表で `Consensus Graph` を正とし、**「`Core Graph` は旧称です。履歴説明以外では契約語彙として再導入しません」** と定めている。`architecture.html` も「契約語彙へ再導入してはならない（履歴注記用途のみ許可）」と重ねて述べている。
  ...
  つまり **`domain.md` が禁じた形式が、`domain.md` を実装する層で4〜5箇所生きている。** `DOC-NORM-01` で扱った「主張した保護範囲 > 実際の保護範囲」と同じ構造である。規範は書かれ、下位層はそれを守っていない。
...
- [ ] 上表の5箇所が `Consensus Graph` になっている
- [ ] `ADR-0028` D11 の改名記録は旧称のまま残っている
- [ ] `CE0-CG-WRITE-IF` の契約IDが変わっていない
- [ ] `ADR-0028` D6 のドリフト検知コマンドが新語で検知でき、かつ旧語の再出現も検知する
- [ ] `docs_contract_checks.py` が廃止語彙の契約文脈での使用を検出する
- [ ] **検出力の確認**: 契約文へ `Core Graph` を1件戻す probe を置き、検査が落ちることを確かめる（是正後に緑であることだけを確認しない）
