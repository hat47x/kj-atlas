...
- [ ] AC-6a: 分岐開始時のキャンバス復元と探究DAG追加を、一つの取り消し操作として整合して戻せる。
...
- これによりAC-4とT6/T6bを完了とした。比較表示がないためAC-5、SafeMode派生共有と保持・削除がないためAC-11は未完了とし、この時点では代表規模の読込が100ms基準を超えたためAC-12も未完了としていた。support levelは`L0: Planned`を維持する。

### T6c 代表規模の容量・読込時間（2026-07-18）

- `ADR-0046`の代表規模を1スナップショット当たり300カード・30島・299関係とし、起点を含む7スナップショット、6ラウンド、1,800カード系譜の決定的fixtureを追加した。既存キャンバス性能E2Eも同じfixtureを参照し、規模定義のずれを防ぐ。
- Chromium実測では1成果は73,955 bytes、探究manifestは2,161 bytes、自己完結bundleは1,460,390 bytesで、5MiBの回帰上限以内だった。JSON集約の容量だけを理由に、Phase 3前に圧縮または差分格納へ移る必要はない。
- 単一・並列実行でNode側の書き出しは101.55から143.59ms、厳格読込は71.13から120.38ms、画面読込完了は1,033.26から1,246.09msだった。これらは単一環境の回帰指標でありSLAではない。
- 画面読込の最大長時間タスクは243から273msで、それ以前にも294msを観測した。`ADR-0046` PB-3の100ms目安を満たさないため、`PERF-INQUIRY-01`を起票し、worker化または段階処理、待機表示、中止境界を実装するまでPhase 3のbackend永続化へ進まない。
- T6cは「計測と判断」を完了した。その後`PERF-INQUIRY-01`でstrict validationとdigest照合をworkerへ移し、読込中表示と中止境界を追加した。代表規模E2Eは単独実行でUI読込1.18から1.45秒、最大長時間タスク93から95msとなり100ms目標を通過した。並列CIでは125msを観測するため150msを退行上限とする。マウス・キーボード・390pxの既存E2Eと合わせてAC-12を完了とする。新しい設計判断は不要で、既存`ADR-0046`と`ADR-0057`に従う。
