...
- これによりAC-4とT6/T6bを完了とする。比較表示がないためAC-5、SafeMode派生共有と保持・削除がないためAC-11、代表規模の読込が100ms基準を超えるためAC-12は未完了のままとする。support levelは`L0: Planned`を維持する。
...
- T6cは「計測と判断」を完了した。AC-12は代表規模E2Eが暫定500ms上限を監視する状態に留まり、100ms以下へ改善するまで未完了とする。新しい設計判断は不要で、既存`ADR-0046`と`ADR-0057`に従う。
