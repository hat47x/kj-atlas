...
- [~] AC-6: `GET /session/context`とactive tenant変更がmembership allowlistだけを返し、自由入力tenantの発見・切替を許可しない。— `switch_tenant_session_context`→`select_active_tenant_context`（membership 検証）・`test_session_context_routes.py` 70 tests pass で確認（allowlist のみ返す・未知/非membership tenant 拒否・stale version 拒否）。**2026-08-11の設計監査で一部撤回**: 上記は単一requestの範囲でしか正しさを示していない。共有ストア`saas_tenant_sessions`は`principal_id`を主キーに`session_version`だけを保持し`selected_tenant`を保存しないため、切替成功responseの直後でも次の`GET /session/context`はBearer tokenのtenant claimから旧tenantへ再解決され得る。「1認証セッション=1 active tenant」というADR-0061の実装gateを満たさない。詳細・受入条件は`issue-SAAS-TENANT-SESSION-BINDING-01-principal-keyed-session-state.md`（`ADR-0074`のAccepted待ち）。同issueの解決までAC-6は未充足として扱う。**2026-08-22時点の是正**: `SAAS-TENANT-SESSION-BINDING-01`のAC-1〜6は、BFF cookie経路（`auth_session_key_hash`解決経路）に限り解決済み——切替後の次requestは保存済みactive tenantを正本として選択先を解決し、多タブのstale requestは409で停止する。**しかし現行SPAが実際に使うBearer token経路はこの修正の対象外**であり、依然principal単位版の旧storeを使う。cookie経路への切替（同issueAC-9・cutover）が完了するまで、本ACは`[~]`（部分・未充足）のまま維持する。
- [ ] AC-7: Workspace、Tenant Admin、Platform Control Planeのcapability/audienceが分離され、Platform operatorに文書readが暗黙付与されない。— **2026-08-25是正**: `ADR-0072`は2026-08-13に採択済み（Accepted）、`issue-SEC-ADMIN-PLANE-01`も2026-08-15完了（Done）であり、本AC単体の着手を妨げていた前提はすでに解消している。ただし`SEC-ADMIN-PLANE-01`が実装したのは`/admin/provision/**`自体の認可（静的admin bearer・fail-fast・越境negative matrix）であり、**本ACが求めるWorkspace/Tenant Admin/Platform Control Plane間のcapability/audience分離（Platform operatorへの文書read暗黙付与防止）はまだ実装されていない**——別の課題である。この分離は既存の認可境界（capability claim、`effectiveCapabilities`解決）に踏み込む安全境界変更であり、着工にはAGENTS.md §1.1の三要素牽制設計法を要すると判断する。本session内では三要素分析・ADR化を行わず、着手可否をMaintainerの判断に委ねる。
