...
- [ ] AC-7: Workspace、Tenant Admin、Platform Control Planeのcapability/audienceが分離され、Platform operatorに文書readが暗黙付与されない。— `ADR-0072`（Proposed、実装先は`issue-SEC-ADMIN-PLANE-01`）が管理面認可方式そのものを決定する前提のため、本AC単体では着手しない（`ADR-0072`「採択前に実装へ着手しない」）。
