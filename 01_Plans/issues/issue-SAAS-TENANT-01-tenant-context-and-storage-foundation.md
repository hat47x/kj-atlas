...
- 変更はtestと`api.md` §10の契約記述のみで、production codeは無変更である。header名`KJ-Atlas-Tenant-Session-Version`の単一性、`409 tenant_session_changed`の値非反射と自動再送禁止、fail-closed既定、single-tenant profileの互換動作、SafeMode既定ON、proposal-only、human-only `human_reviewed`、`KJ_ATLAS_LLM_PROVIDER=none`はいずれも変更していない。
...
- 変更はtest 1ファイル（`03_Implement/frontend/src/api/client.test.ts`）のみで、production codeとrequest挙動は無変更である。single-tenant profileでheaderを送らない既存挙動、SafeMode既定ON、proposal-only、human-only `human_reviewed`、`KJ_ATLAS_LLM_PROVIDER=none`、fail-closed tenant checkはいずれも変更していない。
...
- 理由コードは`access_control.py`の`FailSafeReason`へ`adapter_missing`として追加した。同列挙の既存adapter系が`adapter_error`という短い接頭辞であるため、`api.md` §8.3の「adapter例外/想定外応答: `adapter_error`」と対になる名前にした。access-control理由コードの命名規約文書と、これに近い既存名は探した範囲では存在しない。`api.md` §8.3のfail-safeマトリクスは`KJ_ATLAS_ACCESS_CONTROL_FAIL_SAFE_MODE`に従って倒れる4系統の表であり、本条件はmodeに関係なく常にdenyするため同表へは追加していない（既存のtenant境界3理由も同表に無く、扱いを揃えた）。
- **正しく設定された稼働中のSaaS deploymentで許可される操作は変えていない**。上記(a)(b)が両立する限り追加分岐は到達不能であり、今回入れたのは同じ保証をenforcement点へ直接書き下したdefense-in-depthである。`ADR-0059`は既に「adapter欠損、`noop`、PDP timeout、無効応答、tenant解決不能では**起動またはrequestを**fail-closedにする」と決めており、今回はそのrequest側を明示しただけであるため新規ADRは追加していない。single-tenant（`local-dev`／`evaluation`／`enterprise-production`）でのadapter未設定Document roundtrip許可、`access_control_fail_safe_mode`の扱い、外部PDPの評価内容、SafeMode既定ON、proposal-only、`KJ_ATLAS_LLM_PROVIDER=none`はいずれも無変更である。auth edge／identity解決層（`SAAS-TENANT-AUTHEDGE-01`所管）にも触れていない。
...
- 全体suiteは`.venv/Scripts`をPATHへ前置して実行する必要がある。前置しない起動では`subprocess.run(["alembic", ...])`でmigrationを呼ぶtest群15件が`alembic`実行体を解決できず失敗する（2026-07-29チェックポイント既述の同じ問題のWindows版）。回帰の有無はこの条件でも別途確認した: 変更後633 passed・15 failedに対し、未変更のmain checkout（`C:\GIT\kj-atlas`）が**同一の15件を同一test名で失敗させ632 passed**となり、差はnew testの1件だけで新規失敗は無い。
...
- PostgreSQL実地検証: WSL Docker（v28.3.3）で使い捨て`postgres:16-alpine`コンテナ（ホストport 15432）を起動し、migration owner役の`postgres`superuserと、superuser属性・`BYPASSRLS`を持たない`kj_atlas_runtime`roleを分離して作成、`alembic upgrade head`（25 revisions）を適用した。`KJ_ATLAS_RUN_PG_RLS_TESTS=1`で`tests/test_document_access_rls_postgres.py`5件（新規1件を含む）が全passし、上記の2種の変異検証も実施した。検証専用コンテナは本チェックポイント記録後に削除する。
