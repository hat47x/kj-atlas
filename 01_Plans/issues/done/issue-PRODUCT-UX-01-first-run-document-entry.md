...
- Related ADR/Spec: `01_Plans/adr/ADR-0031-productization-screen-information-architecture.md`, `01_Plans/issues/done/issue-MVP-EXIT-01-productization-readiness.md`, `ROADMAP.md`
...
- AcceptanceScenario（前提 / 操作 / 期待結果 / 除外）: 前提=ブラウザでsui-sensemakingを初回起動する / 操作=新規作成、サンプルを開く、document.jsonを読み込む、SafeMode状態を確認する / 期待結果=内部管理用語やレガシー導線に迷わず、現在の作業開始方法が分かる / 除外=認証付きポータルやクラウド同期の実装。
