...
- Related ADR/Spec: `01_Plans/issues/done/issue-DOGFOOD-13-island-summary-grounding-capped-at-three-cards.md`（同じく接地のモック盲点）, `02_Architecture/api.md`（summarize-island-relation 契約・groundingCardIds は allowed の部分集合）, `00_Prompt/sensemaking_technique.md` §4（島間関係線と接地）
...
- **接地は島間関係の根拠（grounding）の正本**（sensemaking_technique.md §4）であり、その保全が検証されない。
