...
- **appRevision**: ビルド時の revision 注入パイプラインが未整備のため、既存の `SUI_` env prefix 慣例を再利用し `SUI_APP_REVISION`（任意・安全な文字種/長さを検証、不正時は `unknown`）から読む。新しい revision 注入基盤は本Issueのスコープ外。
