...
- RequirementStatement: public runtime configuration keys are exhaustively documented and every public environment variable starts with `SUI_`.
- AcceptanceScenario（前提 / 操作 / 期待結果 / 除外）: 前提=ADR-0021 and runtime registry are accepted; 操作=Compose build, backend settings load, frontend build, and docs-check are executed; 期待結果=only `SUI_*` keys are exposed to users and all accepted keys are documented; 除外=changing SafeMode/share/export policy.
- SecurityGateImpact（SafeMode / share-export / import-sanitize / public-exposure）: public-exposure

## 1) 課題 / Problem statement

- The project policy now requires all public environment variables to use `SUI_` without exception.
...
- Backend settings and the runtime registry previously diverged on `SUI_CE4_STUB_UNRESOLVED_CONTRACTS`.
...
- The latest documentation work aligns the public docs, frontend build, Compose input keys, and backend settings with the `SUI_*` contract.
...
- Treat `SUI_*` as the only public environment-variable namespace.
...
- Keep backend `Settings` validation aligned with the registry, including `SUI_CE4_STUB_UNRESOLVED_CONTRACTS`.
...
- Do not expose non-`SUI_*` compatibility keys in public docs.
...
- [x] Public docs and run guides do not instruct users to set non-`SUI_*` environment variables.
- [x] Frontend build can be configured with `SUI_FRONTEND_API_BASE`.
- [x] Docker Compose public inputs use `SUI_WEB_PORT`, `SUI_POSTGRES_DB`, `SUI_POSTGRES_USER`, `SUI_POSTGRES_PASSWORD`, and `SUI_FRONTEND_API_BASE`.
...
- [x] Backend settings and `runtime_parameter_registry.md` agree on all `SUI_CE4_*` keys, including legacy-key rejection behavior.
...
- [x] T2: Reconcile Compose public input keys with the `SUI_*` contract.
...
- 期待結果:
  ...
  - frontend build reads `SUI_FRONTEND_API_BASE`.
  - public docs and registry no longer expose non-`SUI_*` settings.
- 未実施時の理由・代替検証:
...
- ロールバック手順: restore previous key mapping only with an explicit ADR or migration decision; do not silently reintroduce public non-`SUI_*` keys.
...
- Public contract keys are frozen to the SSOT set in `02_Architecture/runtime_parameter_registry.md` (`SUI_*` only).
...
- "Strict interpretation" (ban non-`SUI_*` in every process env) is tracked as a separate architecture change, not as a drift-fix interpretation of this issue.

## 11) Stream C execution snapshot (2026-05-17)

### Public key extraction result

- `02_Architecture/runtime_parameter_registry.md` and `04_Documentation/configuration.md` expose the same 42 public keys (`SUI_*` only).
- Compose/public runtime inputs are constrained to `SUI_WEB_PORT`, `SUI_POSTGRES_DB`, `SUI_POSTGRES_USER`, `SUI_POSTGRES_PASSWORD`, `SUI_FRONTEND_API_BASE`, and backend runtime keys remain `SUI_*`.
...
1. Governance decision: whether to require zero non-`SUI_*` names in every process environment, including third-party containers.
...
- Naming: 公開キーは引き続き `SUI_*` のみを許容し、追加の互換キー導入は行わない。
- Defaults: `SUI_ALLOW_JIT_PROVISIONING`（実装既定 `true`）と enterprise 推奨値（`false`）の差を registry と運用文書で明示した。
- Compatibility: `SUI_ACCESS_CONTROL_FAIL_SAFE_MODE` の既定値と enterprise 運用時の選択肢（`read_only`/`deny`）を文書間で同期した。
...
- `SUI_FRONTEND_API_BASE` の公開契約を「path（`/` 始まり）」へ明文化し、`runtime_parameter_registry.md` と `deployment.md` の記述を一致させた。
...
- backend/compose の公開キー面には追加ドリフトを検出しなかった（`SUI_*` 契約維持）。
...
- Phase 5 Verify: 公開キー契約（`SUI_*` のみ）と `SUI_FRONTEND_API_BASE` の path 契約が維持されることを文書整合で確認。
...
- Naming: 公開環境変数の命名は `SUI_*` に統一され、例外は third-party container 内部名に限定される（公開契約外）。
- Defaults: `runtime_parameter_registry.md` の既定値記述と issue 側の受入意図に矛盾なし（特に `SUI_FRONTEND_API_BASE=/api` と CE4/ACl 系の安全側既定）。
...
- Context: ENV prefix migration は backend で互換なし一括移行済み、deploy 層は公開キー `SUI_*` と内部 adapter 名（PostgreSQL）を分離して運用している。
...
| --- | --- | --- | --- | --- |
| `SUI_WEB_PORT` | Defined (`8080`) | Defined | Used in `web.ports` | Aligned |
| `SUI_POSTGRES_DB` | Defined (`sui_sensemaking`) | Defined | Mapped to `db.POSTGRES_DB` | Aligned |
| `SUI_POSTGRES_USER` | Defined (`sui_sensemaking`) | Defined | Mapped to `db.POSTGRES_USER` | Aligned |
| `SUI_POSTGRES_PASSWORD` | Defined (`sui_sensemaking`) | Defined | Mapped to `db.POSTGRES_PASSWORD` | Aligned |
| `SUI_FRONTEND_API_BASE` | Defined (`/api`) | Defined | Used in `web.build.args` | Aligned |
| `SUI_DATABASE_URL` | Defined | Defined | Used in `api.environment` | Aligned |
| `SUI_LLM_PROVIDER` | Defined (`none`) | Defined | Used in `api.environment` | Aligned |
...
- 優先順位を以下に固定:
  1. **セキュリティ影響キー**: `SUI_ACCESS_CONTROL_*`, `SUI_AUDIT_*`, `SUI_API_KEY`
  2. **起動不能リスクキー**: `SUI_DATABASE_URL`, `SUI_LLM_PROVIDER`, `SUI_POSTGRES_*`
...
| --- | --- | --- | --- |
| Public naming | Public key は `SUI_*` のみ | backend settings aliases は `SUI_*` のみ、legacy key は reject | None |
| Legacy compatibility | 旧キー互換なし（fail-safe） | `LEGACY_ENV_KEYS` 検知時に `ValueError` | None |
| Frontend API base key | `SUI_FRONTEND_API_BASE`（default `/api`、path契約） | Dockerfile ARG/ENV と `client.ts` の `/api` fallback 正規化 | None |
| Compose public inputs | 利用者入力は `SUI_*` | `web/api/db` の公開入力は `SUI_*` | None |
...
2. third-party container 内部を含む「全 process env で非 `SUI_*` 禁止」を採用するか（採用時は deployment 再設計が前提）。
...
- Compose 内 `POSTGRES_*` は内部変換先であり、利用者入力は `SUI_POSTGRES_*` だけを維持する。
...
- 後方互換は「backend 互換なし」「frontend key は `SUI_FRONTEND_API_BASE` 正規」を前提に逸脱なし。
- Self-heal policy（max 3）: 今回は不一致検知 0 件のため修復ループ未使用。

#### Phase 6: Proceed（切替 / ロールバック / 監視）

- 切替手順:
  1. 運用者が設定する env を `SUI_*` のみに限定する。
  2. Compose 起動前に `SUI_POSTGRES_*` と `SUI_DATABASE_URL` の整合を確認する。
  3. frontend build では `SUI_FRONTEND_API_BASE` を `/` 始まり path のみで設定する。
- ロールバック手順:
  1. 新規設定投入後に起動不全が発生した場合、直前の `SUI_*` 設定セットへ復元する。
  2. 旧キー復活は行わず、registry の契約値へ修正して再展開する。
- 監視ポイント:
  - Compose config 生成時に `SUI_*` 以外の公開キーが runbook/ops に混入していないこと。
  - `SUI_FRONTEND_API_BASE` が不正値（空や `/` 非始まり）で運用されていないこと。
...
- 命名整合を確認: 公開キーは `SUI_*` のみ。
- 既定値整合を確認: `SUI_FRONTEND_API_BASE=/api`、`SUI_WEB_PORT=8080`、`SUI_LLM_PROVIDER=none` 等が一致。
...
- Verify: 公開契約キーが `SUI_*` のみであること、profile 表と新設判断基準が矛盾しないことを文書差分で確認。
...
### Phase 2: ゲート定義（E1-E3）
- **E1 Public key contract**: 公開文書・runbookに非 `SUI_*` を公開設定として記載しない。
- **E2 Runtime validation**: backend settings が無効値（adapter/fail-safe/endpoint）を検出し、許容時は ADR 根拠を要求。
- **E3 Compose consistency**: `SUI_*` 入力と compose 展開結果が一致し、vendor env 名は private boundary に閉じる。
...
- 公開設定は `SUI_*` のみ、vendor 名は private adapter boundary のみという解釈で一致。
...
- 公開キー集合は `SUI_*` のみを維持（registry / configuration / compose inputs）。
...
- Allowlist 対象（registry/deployment/enterprise/configuration/operations/security guidelines/issue群/ADR-0021）を再読し、公開キーが `SUI_*` に統一されていることを再確認。
...
- プロファイル安全既定値: `enterprise-production` では `SUI_ALLOW_JIT_PROVISIONING=false` と fail-safe (`read_only`/`deny`) を運用上の必須確認項目として維持。
...
| 論点 | 判定 | 理由 |
| --- | --- | --- |
| 公開環境変数の命名 | Pass | 利用者・運用者が設定する公開キーは `SUI_*` のみ。 |
...
| 全 process env からの非 `SUI_*` 排除 | Pending by design | 採用する場合は deployment 再設計を伴う別ADRが必要。 |
| `external_http` endpoint 未設定時の fail-fast 既定化 | Pending by design | 可用性/安全性トレードオフを変更するため、この issue だけでは決めない。 |

### Done readiness

- This issue is close to Done for the public runtime contract, because the registry, public documentation, Compose inputs, and backend settings contract now point to the same `SUI_*` namespace.
...
- Reviewed: key scan for `VITE_API_BASE`, `POSTGRES_*`, `DATABASE_URL`, `WEB_PORT`, and `FRONTEND_API_BASE` showed public docs/config using `SUI_*`; `POSTGRES_*` appears only in private-boundary documentation or Compose adapter mapping.
...
- Finding: product runtime public keys already used `SUI_*`, but local verification harness keys still had non-prefixed PostgreSQL opt-in, provider profile, and Auth Level2 names.
- Decision: verification harness keys are not public runtime settings and remain out of `04_Documentation/configuration.md`; however, the "no prefix exception" rule still applies to names used by sui-sensemaking scripts and tests.

### Execute

- Renamed PostgreSQL roundtrip opt-in to `SUI_RUN_PG_TESTS`.
- Renamed Auth Level2 harness inputs and derived URLs to `SUI_AUTH_LEVEL2_*` and `SUI_AUTH_PROVIDER_PROFILE_DIR`.
...
- Added `test_project_env_access_points_use_sui_sensemaking_prefix` to keep backend/frontend direct env access points from reintroducing non-prefixed project keys.

### Verify

- Pass: legacy-name scan over `01_Plans`, `02_Architecture`, `03_Implement`, and `04_Documentation` -> no matches.
- Pass: direct env-read scan over backend/frontend/deploy found no non-`SUI_*` env reads, excluding `import.meta.env.DEV`.
...
- Finding: the implementation and public documentation keep the public runtime contract on `SUI_*`, but `ADR-0021` had become hard to read and could no longer serve as the decision record for the no-exception prefix policy.
...
- Rewrote `ADR-0021` in readable Japanese while preserving the Accepted decision: public sui-sensemaking environment variables must start with `SUI_` without exception.
- Clarified that `POSTGRES_*` and build-tool internal names are private adapter boundary names, not user-facing sui-sensemaking configuration.
...
- Finding: current implementation, registry, and public docs use `SUI_*`, but several older accepted ADRs still referenced `DATABASE_URL`, `LLM_PROVIDER`, `LLM_ESCALATION_ENABLED`, or `LLM_LARGE_SCALE_OPT_IN` as if they were current setting keys.
...
- Decision: normalize those references to the accepted `SUI_*` names without changing the historical ADR decisions or product runtime behavior.

### Execute

- Updated `ADR-0001`, `ADR-0003`, `ADR-0007`, `ADR-0009-local-llm-integration`, and `review-ADR-0009-phase-a` to use current `SUI_*` key names.
...
- Finding: `04_Documentation/configuration.md` already listed the concrete PostgreSQL Compose keys in the full environment-variable table, but the earlier boundary example still used `SUI_POSTGRES_*` as a shorthand.
- Decision: public user-facing configuration examples should not use shorthand wildcard names when the requirement is to list every supported environment variable. The general policy phrase `SUI_*` remains acceptable for describing the namespace rule, but concrete configuration examples must enumerate exact keys.

### Execute

- Replaced the public-boundary example `SUI_POSTGRES_*` with `SUI_POSTGRES_DB`, `SUI_POSTGRES_USER`, and `SUI_POSTGRES_PASSWORD`.
...
- Added a guard that rejects concrete-key wildcard shorthand such as `SUI_POSTGRES_*` in the public configuration document, while still allowing the namespace policy phrase `SUI_*`.
...
- Canonical public runtime keys remain `SUI_*` only. Legacy unprefixed keys are rejection targets, not compatibility aliases.
- External LLM escalation remains default-off: `SUI_LLM_PROVIDER=none`, `SUI_LLM_ESCALATION_ENABLED=false`, and `SUI_LLM_LARGE_SCALE_OPT_IN=false`.
...
| --- | --- | --- | --- |
| Backend runtime keys | Registry, configuration doc, and `Settings` aliases use `SUI_*`. | None | Marked AC/T1-T7 complete. |
| Legacy backend keys | `LEGACY_ENV_KEYS` rejects unprefixed historical names. | None | Verified by backend prefix tests. |
| CE4 keys | Registry and settings agree on `SUI_CE4_*`, including `SUI_CE4_STUB_UNRESOLVED_CONTRACTS`. | None | Verified by backend prefix tests. |
...
| Frontend API base | `SUI_FRONTEND_API_BASE` is the public build key and invalid values fall back to `/api`. | None | Verified by frontend API client unit test and typecheck. |
| Compose public inputs | Compose public inputs are `SUI_WEB_PORT`, `SUI_POSTGRES_DB`, `SUI_POSTGRES_USER`, `SUI_POSTGRES_PASSWORD`, `SUI_FRONTEND_API_BASE`, `SUI_DATABASE_URL`, and `SUI_LLM_PROVIDER`. | None | Verified by static Compose review; Docker binary unavailable for live config expansion. |
| Private adapter names | `POSTGRES_DB`, `POSTGRES_USER`, and `POSTGRES_PASSWORD` appear only as PostgreSQL container internal names mapped from `SUI_POSTGRES_*`. | Accepted boundary | Keep under ADR-0029; no public alias added. |
...
- Deploy examples and docs continue to show user-set keys with `SUI_` only.
...
- Frontend env reading is limited to `SUI_FRONTEND_API_BASE`.
...
- Compose does not embed secrets; PostgreSQL defaults are demonstrative local/evaluation values and remain configurable through `SUI_POSTGRES_*`.
...
- Registry alignment: public user-facing keys are `SUI_*`; private adapter names remain non-public.
...
- Command exits 0.
- User-facing inputs in the rendered config are driven by `SUI_WEB_PORT`, `SUI_POSTGRES_DB`, `SUI_POSTGRES_USER`, `SUI_POSTGRES_PASSWORD`, `SUI_FRONTEND_API_BASE`, `SUI_DATABASE_URL`, and `SUI_LLM_PROVIDER`.
- `POSTGRES_DB`, `POSTGRES_USER`, and `POSTGRES_PASSWORD` appear only under the PostgreSQL third-party container as private adapter names mapped from `SUI_POSTGRES_*`.
- No additional public non-`SUI_*` setting is introduced.

If the command fails because Docker is unavailable, keep this issue `In Progress`. If it fails because the rendered config exposes a new public non-`SUI_*` input or cannot expand the current public keys, treat that as new drift and update this issue before closing it.

## 25) Legacy frontend key documentation guard (2026-06-19)

### Finding

- The frontend implementation already reads only `SUI_FRONTEND_API_BASE` through Vite's `SUI_` prefix.
...
- Clarified that the frontend build reads only `SUI_FRONTEND_API_BASE` and does not provide a legacy frontend-key shim.
...
- Proceed: the documented contract now matches the frontend implementation and the `SUI_*`-only policy.
