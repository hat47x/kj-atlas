...
- 新規spec: 実backend起動（fresh SQLite、`alembic upgrade head`、`SUI_LLM_PROVIDER=none`）に対し `SUI_E2E_REAL_BACKEND=1 npx playwright test e2e/ai_model_ux_available_models_reason.spec.ts --workers=1` で2件pass。`SUI_E2E_REAL_BACKEND`未設定では2件ともskip（既定の`npm run e2e`はbackend不要のまま）。
...
- `01_Plans/docs_check.py`: 新規env var `SUI_E2E_REAL_BACKEND`・`SUI_E2E_BACKEND_URL`を`02_Architecture/runtime_parameter_registry.md`へ追加し、pass確認。
