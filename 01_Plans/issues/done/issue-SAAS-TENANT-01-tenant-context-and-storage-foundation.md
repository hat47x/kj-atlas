...
- [x] AC-4: SaaS profileでtenant不明・不一致、membership停止、adapter欠損、PDP不達をreadも含めてdenyする。— **2026-09-05完了確認**: `test_saas_e2e_tenant_isolation.py`で未知tenant・claim欠落・越境same-docIdをHTTP境界で拒否し、`test_session_context_routes.py`でmembership停止・membership差替え・capability resolver欠損をfail-closedにしている。加えてPR #2909の`test_saas_document_read_pdp_outage.py`が、server-resolved SaaS tenant sessionから実際のDocument GETを通し、`ExternalPolicyAccessControlAdapter`のtransport `URLError`を403 `policy_ref_unreachable`としてfail-closedにし、deny応答にDocument title/card bodyが含まれないことを固定した。これにより、tenant不明・不一致、membership停止、adapter欠損、PDP不達をreadを含めてdenyする現行BFF境界の証拠が揃った。
...
- [x] AC-6: `GET /session/context`とactive tenant変更がmembership allowlistだけを返し、自由入力tenantの発見・切替を許可しない。— **2026-09-04再監査**: 2026-08-22の「現行SPAはBearer token経路」という停止理由は、Accepted済み`ADR-0074`とその後のBFF cutoverにより失効した。`saas-multitenant` frontend entryは`TenantSessionRuntimeGate`を必須とし、server-owned cookie sessionの`/session/context`成功後だけtenant-scoped Appをmountする。active tenantは認証session keyに束縛され、切替はmembership allowlist再照合＋`tenantSessionVersion`比較を経て保存される。`test_session_context_routes.py`は未知/非membership tenant、停止membership、stale version、次requestのsession bindingを固定している。
- [x] AC-7: Workspace、Tenant Admin、Platform Control Planeのcapability/audienceが分離され、Platform operatorに文書readが暗黙付与されない。— **三要素牽制設計（2026-09-04）**: データ面ではtrusted `TenantSessionContext`に全11 capabilityを保持してserver-side再認可へ使い、browser向けWorkspace session responseはWorkspace 4種＋Tenant Admin 5種だけへ射影する。機能面では`/admin/provision/**` Stage Bが引き続きserver-owned sessionの`tenant.provision`を直接要求し、`/session/context`とactive-tenant responseは`tenant.provision/suspend`を露出しない。業務ルール面では3 capability集合を互いに素・全既知集合の完全分割として固定し、Platform集合に`document.read`/`document.policy.manage`を含めない。`test_capability_audience_separation.py`とsession/control-plane/Tenant Admin近接testをRun 33876867421で検証し、88件pass・Ruff pass。JWT `aud`やBFF session schemaは変更していない。
...
- [x] AC-13: 同じ認証セッションの複数タブ、同時tenant切替、bfcache復帰、遅延responseで古い`tenantSessionVersion`を持つ現存tenant-scoped操作がresource lookup/commit前に拒否され、client通知が欠落しても新tenantへ自動再送・commitされない。— **2026-09-06 Lane C完了確認（GitHub Actions Run `34021273534`）**: 既存`tenant_session_multitab.spec.ts`がBroadcastChannel欠落、bfcache復帰、遅延response、stale PUT 409非再送、旧DOM/cache破棄をreal Chromiumで固定し、新規`tenant_session_real_backend_multitab.spec.ts`が同じHttpOnly認証sessionを共有する2 tabをmock Broker→実frontend→shared PostgreSQL→2 backend workerへ通した。tab Aのtenant A/V1→tenant B/V2切替後、tab Bのstale V1 GETは存在しないdocに対して404より先に409 `tenant_session_changed`となりresource lookup前拒否を黒箱で証明し、stale V1 PUTも409、V2で同docが404のままなのでcommit非発生を確認した。stale V1によるtenant Aへの切戻しも409で、最終sessionはtenant B/V2を維持した。実stack側ではCSRFを弱めずbrowser-visible Hostをproxy/gateway越しに保持し、E2E fixtureにはtenant B membership＋IdP bindingだけを追加した。Run `34021273534`でfrontend typecheck、harness Ruff、既存auth回帰＋AC-13のChromium 2/2がpassした。

### Lane C checkpoint 2026-09-06: AC-13 real-backend multi-tab stale-session acceptance

- 三要素牽制では、業務面を「旧tabの操作を新tenantへ自動再送・commitしない」、データ面を「active tenant/versionはserver-owned auth sessionを正本としclient tenant値を認可根拠にしない」、機能面を「trusted session再解決後かつresource lookup/commit前の`tenantSessionVersion` preconditionを実backendで観測する」とした。CSRFやtenant検証を弱めるtest-only production hookは追加していない。
- `03_Implement/frontend/e2e/tenant_session_real_backend_multitab.spec.ts`を追加した。同一BrowserContextの2 tabが同じHttpOnly sessionでtenant A/V1を共有し、tab Aがtenant B/V2へ切替後、tab BのV1 GET/PUT/切戻しを409 `tenant_session_changed`で拒否する。存在しないdocへのstale GETが404より先に409になることでresource lookup前、stale PUT後にV2 GETが404のままであることでcommit前の拒否をそれぞれ黒箱で固定した。
- 実認証縦断fixtureはtenant Bのactive membershipと同じIdPへのtrusted bindingを追加した。cookie session復元後もtenant/IdP/membershipを再検証するproduction契約を迂回せず、fixture側だけで正しい第2tenantを成立させた。
- 実browserのunsafe BFF requestがCSRFのOrigin/Host一致を満たすよう、Vite `/api`・`/session` proxyとdeterministic E2E gatewayはbrowser-visible Hostを保持する。CSRF middleware、cookie属性、tenant-session precondition自体は変更していない。
- multi-instance専用Playwright configは既存auth specと新AC-13 specを1 workerで直列実行する。共有fixtureを2 workerが同時利用した際のDevTools response-body競合を受入条件へ混入させないためであり、製品の並行request処理を直列化する変更ではない。
- GitHub Actions Run `34021273534`（Ubuntu hosted runner、PostgreSQL 16、Chromium）でfrontend `tsc --noEmit`、E2E harness `ruff check`、`run_saas_browser_multi_instance_e2e.sh`を実行し、既存auth回帰＋新AC-13の2/2がpassした。検証専用workflowは成功後にbranchから削除し、恒久CIへ追加していない。
- 既存`tenant_session_multitab.spec.ts`のmock API real-browser matrixがBroadcastChannel欠落、bfcache、遅延response、旧DOM/cache、非再送を担い、新specが従来欠けていた同一実auth session＋2 tab＋実backend/共有PostgreSQL軸を担う。両証拠を合わせてAC-13の現存surface要件を充足したと判断する。
...
- GitHub Actions Run `33964657276` で、backendのsame-docId HTTP matrix、MCPのSaaS fail-fast、frontend generation guard、real Chromium SaaS tenant-switch/storage scenarioを同一Runとして再実行した。production code/API/schemaには変更を加えていない。
...
### Implementation checkpoint 2026-09-04: capability authorization surface separation

- `ADR-0067`の三要素牽制設計を適用し、capabilityをWorkspace 4種、Tenant Admin 5種、Platform Control Plane 2種へ明示分類した。3集合は互いに素で、和集合は既存11 capabilityと一致する。
- trusted server-side `TenantSessionContext`はPlatform capabilityを失わず、`/admin/provision/**` Stage Bの`tenant.provision`認可へ利用できる。一方、Workspace bootstrap/switch responseの`effectiveCapabilities`はWorkspace＋Tenant Admin 9種だけへ射影し、`tenant.provision/suspend`をbrowserへ露出しない。
- Platform capabilityから`document.read`または`document.policy.manage`を暗黙導出する経路は追加しない。同じ主体が複数面のcapabilityを独立に持つことは許容するが、面をまたぐ継承は行わない。
- JWT `aud`、OIDC provider契約、`ADR-0074`のserver-owned BFF session schemaは変更していない。`api.md`の「認可audience」はapplication route/capability surfaceとして実装する。
- 検証: GitHub Actions Run `33876867421`。`test_capability_audience_separation.py`、session context routes、control-plane authorization、Tenant Admin document-accessの計88件pass、Ruff pass、diff check pass。一時workflowは成功後に自己削除した。
...
- 公開`SUI_RUNTIME_PROFILE`を追加し、`local-dev`、`evaluation`、`enterprise-production`を正規化して受理する。backend直接起動の既定は`local-dev`、Composeの既定は`evaluation`とした。
...
- 独立起動できるMCP processでも`SUI_RUNTIME_PROFILE`を検証し、single-tenant 3 profileだけを受理する。`saas-multitenant`はtenant-bound MCP/agent credentialが未実装のため起動前に拒否し、未知profileも拒否する。
...
- Alembic `20260717_0009`でPostgreSQLの`documents`と`merge_decision_logs`へENABLE/FORCE RLS policyを追加した。policyは`current_setting('sui_sensemaking.tenant_id', true)`と行tenantが一致する場合だけUSING/WITH CHECKを許可し、setting欠落・空値はread/writeとも不許可になる。SQLite migrationはno-opとする。
- tenant-scoped repositoryは各操作前にparameter bindingされた`set_config('sui_sensemaking.tenant_id', tenantId, true)`を実行する。第3引数`true`によるtransaction-local設定のためcommit/rollback後のpool再利用へtenant値を持ち越さない設計とし、任意tenant文字列をSQLへ連結しない。
...
- PostgreSQLではmetadata表にもENABLE+FORCE RLSを設定し、transaction-local `sui_sensemaking.tenant_id`と一致する行だけをread/write対象とする。SQLiteはmigration/単一テナント互換検証用でRLSを適用しない。
...
- trusted HTTPのredirect拒否と組み合わせ、tenant由来promptを別接続先へ転送せず、巨大・拡張応答をproposal処理へ渡さない。`SUI_LLM_PROVIDER=none`既定、local-first、明示opt-in、proposal-onlyは変更しない。
...
- request/responseの`provider_validation`は`SUI_LLM_FALLBACK_TO_NONE=true`でもfallback対象にせず、契約違反をprovider disabledの503へ変換しない。timeout/unavailableに対する既存fallback、既定none、明示opt-in、proposal-onlyは維持する。
...
- frontend buildへ既存`SUI_RUNTIME_PROFILE`を渡し、未指定・`local-dev`・`evaluation`・`enterprise-production`はpolicy通信なしのlocal-first App、`saas-multitenant`だけはserver policy一致→session再検証→tenant scope付きAppの順でmountするentry pointを結線した。未知・空・前後空白を含むbuild値、policy取得失敗・不一致はsingle-tenantへfallbackせず、旧App本文のないblocked stateへ閉じる。
- 公式Composeはbackend environmentとfrontend buildへ同じ`${SUI_RUNTIME_PROFILE:-evaluation}`を渡し、契約testで両delivery surfaceの一致を固定した。現行backend settingsは`saas-multitenant`を引き続き起動前に拒否するため、frontend配線だけでSaaSを解禁しない。
...
- SaaS profileでだけ有効になる共通request guardを追加し、`Sui-Sensemaking-Tenant-Session-Version`を単一headerとして固定した。trusted sessionの現versionを解決した後に照合し、欠損・重複・不正・不一致を値非反射の`409 tenant_session_changed`へ統一する。client headerはidentity、tenant、membership、capabilityの解決には使用しない。
...
- `GET/PUT /docs/{doc_id}`と`POST /docs/{doc_id}/export-audit`へ単一の`Sui-Sensemaking-Tenant-Session-Version`を付与した。clientは呼出時にもsession contractを再検証し、不正contextは通信前に拒否する。serverの`409 tenant_session_changed`はDocument metadata競合として保存再試行へ流さず、runtime resourceをcleanupして旧Appをblocked化する。
...
- 文書内容・source ID・proposal判断を扱う全`POST /ai/*`と`POST /context/*`へ共通dependencyを追加した。SaaS profileではtrusted identity／tenant sessionを再解決し、単一`Sui-Sensemaking-Tenant-Session-Version`をendpoint本体、LLM transport、audit記録、bundle生成より先に照合する。read-onlyな`GET /ai/provider-status`はtenant資源を扱わないため対象外とした。single-tenant profileは既存requestを維持する。
...
- ローカル環境にDocker（v28.3.3）が利用可能であることを確認し、`03_Implement/deploy/docker-compose.yml`の共有DB設定・loopback限定方針（`DEPLOY-NET-01`）には触れず、検証専用の使い捨て`postgres:16-alpine`コンテナ（ホストport 15432、非公開設定と分離）を起動した。README記載の手順どおり、migration所有者ロールと、superuser属性・`BYPASSRLS`を持たない`sui_sensemaking_runtime`ロールを分離して作成した。
...
- 併せて`sui_sensemaking_runtime`ロールへ`ALTER DEFAULT PRIVILEGES`でのsequence USAGE権限が漏れていたため付与し（`merge_decision_logs.id`等の自動採番列に必要）、`test_docs_roundtrip.py`のPostgres roundtripテストがAlembicをsubprocessで呼ぶ際に`alembic`実行体をPATH解決できない既知の問題（2026-07-18チェックポイント既述）を、venvの`bin`をPATHへ前置して解消した。
...
### Implementation checkpoint 2026-08-06: exhaustive registered-route guard coverage

- 2026-08-02／08-03チェックポイントが残課題として繰り返し挙げていた「実trusted auth edge、server側import/share endpoint、Admin/MCP/非同期jobを含む全consumer」を、記述の再掲ではなく現在のコードで全数監査した。結果、4項目は既に対象が存在しないか`saas-multitenant`で到達不能であり、当該残課題記述の一部は陳腐化していた。
  - 登録HTTP surfaceはFastAPI 31 APIRouteで全てである（`/healthz` 1、`/docs/{doc_id}`系 9、`/admin/provision` 2、`/ai` 9、`/context` 4、`/tenant-admin/document-access` 3、`/session` 3）。`backend/src`内の`APIRouter`は7個で全て`main.py`が登録済みであり、未登録router、`Mount`、静的sub-app、第2のASGI appは存在しない。tenant資源を扱う24 routeはいずれも既に`_authorize_request`、`_authorize_document_policy_management`、`require_tenant_scoped_api_precondition`のどれかをresource lookup前に通る。
  - **server側import/share endpointは存在しない**。frontendのimport/shareはFile読取、JSZip展開、clipboard、downloadのbrowser内処理のみで、永続化は既にguard済みの`PUT /docs/{doc_id}`だけを経由する（frontendが要求するbackend pathは13種で、`/import`／`/share`相当は無い）。`api.md` §5.2は完全な共有機能を非MVP、§10はimport／share／MCP／webhook／非同期job開始点を未実装・非公開と明記しており、当該残課題記述は将来endpointを指す。`agent_registrations`もdoc/契約のみで表・endpoint・backend code pathを持たない（`EXT-CONN-02`所管）。
  - **backendに非同期job機構は存在しない**。`BackgroundTasks`、Celery、scheduler、queue consumer、`asyncio.create_task`、worker threadのいずれもproduction codeに無く（`threading.Thread`はtestのmock HTTP serverのみ）、request/response外でtenant資源へ触れる経路が現時点で無い。
  - **MCPは`saas-multitenant`で起動しない**。`index.ts`はstdio／httpどちらのtransportでも`loadDocumentClientConfigFromEnv()`→`validateMcpRuntimeProfile()`を経るため、profile一致時点でthrowし`buildHttpApp`のExpress route登録まで到達しない。よってMCP側per-request guardは現時点で不要であり、tenant-bound MCP credentialは引き続き非目標である。
  - `admin.py`の2 routeも実装内容で判定した。`POST /admin/provision/users`は`require_single_tenant_provisioning_surface`により`saas-multitenant`で404、未知profileで503となる（behavioral testは既存）。`POST /admin/provision/hil-rs/a2a3-gate:validate`はhardcodeされたfrozen valueとpayloadを比較するだけでDB・tenant資源・`get_db`を一切持たないstateless契約検証であり、tenant guardを必要としない。
- 監査で判明した実際の未充足は、guard本体ではなく**guard coverage契約の合成方法**であった。2026-07-20の`registered route authorization coverage guard`は`/ai/`、`/context/`、`/docs/`、`/tenant-admin/document-access`の4 prefixへ`startswith`で一致したrouteだけを検査するため、それ以外のprefixへ登録されたrouteは1つのassertionにも掛からずtestが黙って成功する。同checkpointの狙い（新規routeが共通境界を省略した場合に個別negative test無しで検出する）が4 prefix内に限定されており、既定fail-openであった。残課題として挙がっていたAdmin surfaceや将来のimport/share surfaceは、まさにこの死角へ入る。
- 契約を既定fail-closedへ反転した。`main_app.routes`の全APIRouteを`(method, path)`で列挙し、(a) dependency treeに`require_tenant_scoped_api_precondition`を持つ、または(b) endpoint ASTが`_authorize_request`／`_authorize_document_policy_management`を参照する、のいずれでもないrouteの集合が、理由コード付き明示exemption表と**集合として完全一致**することを要求する。これによりguardを持たない新規routeは分類されるまでtestが落ち、逆に既存exemptionが陳腐化した場合も落ちる。
- exemptionの理由自体も機械的に再検査する。`no-tenant-resource`（`/healthz`、`/ai/provider-status`、`/admin/provision/hil-rs/a2a3-gate:validate`、`/session/bootstrap-policy`）はdependency treeとendpoint ASTの双方に`get_db`が現れないこと、`saas-surface-blocked`（`POST /admin/provision/users`）は`require_single_tenant_provisioning_surface`が実際に配線されていること、`tenant-session-version-source`（`GET /session/context`）と`body-borne-expected-version`（`POST /session/active-tenant`）は`resolve_trusted_saas_request_session`（後者は加えて`require_current_tenant_session_version`）を呼ぶことを要求する。version発行元と本文経由expected versionをheader必須化すると循環するため、この2 routeがheader guardの対象外である理由をtest側へ固定した。
- APIRoute以外のroute entryが`/openapi.json`、`/docs`、`/docs/oauth2-redirect`、`/redoc`というFastAPI自身のschema／docs route 4件だけであることも型と集合で固定した。`Mount`されたASGI sub-appはAPIRoute検査を丸ごと迂回するため、将来import/share surfaceがsub-appとしてmountされた時点で検出される。
- 変更はtestと`api.md` §10の契約記述のみで、production codeは無変更である。header名`Sui-Sensemaking-Tenant-Session-Version`の単一性、`409 tenant_session_changed`の値非反射と自動再送禁止、fail-closed既定、single-tenant profileの互換動作、SafeMode既定ON、proposal-only、human-only `human_reviewed`、`SUI_LLM_PROVIDER=none`はいずれも変更していない。
- 検証: 追加5件を含む`test_tenant_session_precondition.py` 12件pass、backend全体647件pass・条件付き25件skip、`ruff check .` pass、変更対象の`ruff format --check` pass、docs-check pass（active_memos=54、tracked_markdown=474）、active issue validator 54 memos pass、`git diff --check` pass。新contractが実際にfail-closedであることを5 mutationで確認した: (1) 未guardな`POST /import/documents`追加、(2) 旧prefix検査が取りこぼす未guardな`GET /docs`追加（`api.md` §2.4の将来一覧endpointと同じ非slash pathであり、`"/docs/"`のstartswith検査には一致しない）、(3) `Mount("/legacy")`追加、(4) DB到達routeの`no-tenant-resource`誤分類、(5) `require_single_tenant_provisioning_surface`除去。各々該当testが失敗し、復元後に全contractが再度passすることを確認した。
- 環境制約: ローカルでDocker/PostgreSQLを起動していないため`postgres` markerの25件は引き続きskipであり、AC-5の実地検証は2026-07-29の結果に依拠する（今回はRLS関連の変更なし）。本worktreeの`.git`はgitdirをWindows絶対path形式で指すためWSL側`git`から解決できず、`git ls-files`に依存するdocs-checkはWSL-native pathへ同一内容のsnapshot（tracked markdown 474件一致）を作成して実行した。frontendは変更していないため`npm run typecheck`／unit／Playwrightは実行していない。
- 残る未充足は変わらず外部依存側である: 実trusted auth edge（IdP/OIDC/SAML検証とanti-forgery付きsession persister）の実runtime接続、実外部PDP／binding／capability serviceへの到達性検証、実auth edgeを前提とする全consumer越境matrixとE2E。今回の変更はこれらを充足しないため、AC-4/6/7/8/9/10/12/13は未チェックのままとし、`saas-multitenant`のsettings起動拒否も維持する。

### Implementation checkpoint 2026-08-06: frontend tenant session request coverage contract

- 同日のbackend側`exhaustive registered-route guard coverage`と対になるclient側の全数監査を行った。`03_Implement/frontend/src`のproduction moduleで`fetch`を呼ぶのは`api/client.ts`の15か所と`App.tsx`の3か所だけである。後者はfrontend origin上のbundled public pack（`./packs/...`）読込でbackend APIでもtenant資源でもなく、staleness境界は2026-07-20の`stale public pack generation guard`が既に担っている。`api/client.ts`の15 requestは14 (method, path) 組で、backend 31 APIRouteのうち13 pathしか呼ばない。`api/session_context.ts`は`fetch`を持たない純粋なvalidatorであり、`session/tenant_switch_request.ts`もheaderを直接付けず`changeActiveTenant`へ委譲する。したがってheader付与の判断点は`client.ts`の共有helper`tenantSessionPreconditionHeaders`1か所へ既に集約されていた。
- 監査の結論として**production requestの欠落は無かった**。header必須の10 (method, path)（`GET/PUT /docs/{docId}`、`POST /docs/{docId}/export-audit`、`POST /ai/`系7）は全て共有helper経由で付与しており、付与しない4 requestはbackendのexemption表と理由コードまで一致していた: `GET /session/bootstrap-policy`と`GET /ai/provider-status`が`no-tenant-resource`、`GET /session/context`が`tenant-session-version-source`、`POST /session/active-tenant`が`body-borne-expected-version`。backendの第4理由`saas-surface-blocked`（`POST /admin/provision/users`）はfrontendが当該pathを呼ばないため対応物を持たない。`App.tsx`側もguard対象10関数の実call site 15件すべてが`runTenantScopedApiRequest(() => ...)`内で`{ tenantSessionContext: verifiedTenantSession }`を渡していた。
- 実際に未充足だったのはguard本体ではなく**client側のcoverage契約**であった。既存の`client.test.ts`は当時の作者が覚えていたrequestを列挙するだけで、AI mutationの検査は`toHaveBeenCalledTimes(7)`という当時の件数に固定されている。8番目のtenant-scoped requestをこのmoduleへ追加しても、どのassertionにも掛からずtestは黙って成功する。2026-07-20のbackend側prefix検査と同型の既定fail-openである。
- 契約を既定fail-closedへ反転した。frontend production sourceを走査して`fetch`を持つmoduleの集合が`api/client.ts`と`App.tsx`だけであることを固定し、`client.ts`の各`fetch`から (method, path)、囲みexport関数、request initを静的に列挙する。共有helperを参照しないrequestの集合が理由コード付き明示exemption表と**集合として完全一致**することを要求するため、header無しのrequestを追加すると分類するまでtestが落ち、逆にexemptionが陳腐化した場合も落ちる。`${API_BASE}`以外を宛先にする`fetch`、export関数の外に置かれた`fetch`、`App.tsx`側の`./packs/`以外の`fetch`はいずれも列挙時点でerrorになる。
- exemptionの理由自体も機械的に再検査する。`no-tenant-resource`はpathに補間値を持たず、request bodyを送らず、`TenantScopedRequestOptions`を受け取らないこと（clientからtenant資源を指名も同送もできない）を要求する。`tenant-session-version-source`は`parseTenantSessionContext`でclosed-world契約を検証して返す側であること、`body-borne-expected-version`はrequest bodyが`expectedTenantSessionVersion: currentSession.tenantSessionVersion`を含むことを要求する。version発行元と本文経由expected versionをheader必須化すると循環するため、この2 requestがheader対象外である理由をbackend契約と同じ形でtest側へ固定した。加えてheader名`Sui-Sensemaking-Tenant-Session-Version`がclient module内で1リテラル・helper 1か所しか参照されず、他のproduction moduleが名前もconstantも参照しないことを固定した。
- `App.tsx`側のcall siteも同じ契約へ入れた。guard対象関数の非コメントcall siteは全て`runTenantScopedApiRequest(() => `の直後にあり、引数に`{ tenantSessionContext: verifiedTenantSession }`を含むことを要求する。さらに`tenantSessionContext: verifiedTenantSession`の総出現数がcall site数と一致することで、列挙を逃れたcall siteが無いことを示す。
- 明示する限界: 契約は`fetch`を字句的に含むexport関数のsourceを検査するため、private helper経由のrequestはその関数へ帰属する（backend契約が`route.endpoint`だけを見てcalleeを追わないのと同じ）。また**この契約はtenantを隔てているものではない**。backendはSaaS profileでheaderを欠くtenant-scoped requestをresource lookup前に`409 tenant_session_changed`で拒否するため、client側の付け忘れはfail-closedな機能破壊であって越境漏えいではない。今回の追加は将来のdriftに対する検出であり、現時点で悪用可能な経路を修正したものではない。
- 検証はWSL-native検証コピー（`~/kjnative-fe`）で実行した。追加7件を含む`src/api/client.test.ts` 35件pass、frontend全体1,381件中1,380件pass・232 file中230 file pass、`npm run typecheck` pass、`npm run build` pass、docs-check pass（active_memos=54、tracked_markdown=474）、`git diff --check` pass。失敗した2 fileは`src/domain/representative_visual_cue_prototype.test.ts`と`src/import/external_agent_workflow_doc.test.ts`で、いずれも検証コピーがrepository親階層の`02_Architecture/`・`04_Documentation/`を持たない環境起因であり、変更前HEADの`client.test.ts`へ差し替えた同一環境でも同じ2 fileが失敗すること（1,374件中1,373件pass）を確認して回帰でないことを示した。
- 契約が実際にfail-closedであることを7 mutationで確認した: (1) header無しのexport関数`postContextAudit`（`POST /docs/{docId}/context-audit`）追加、(2) `session/runtime_activation.ts`へbackend宛`fetch`追加、(3) `App.tsx`のexport-audit call siteからsession option削除、(4) `no-tenant-resource` exemptionがrequest bodyを送るよう変更、(5) `App.tsx`のnarrative生成call siteが`runTenantScopedApiRequest`を経由しないよう変更、(6) `GET /session/context`が`parseTenantSessionContext`検証をやめる、(7) `generateNarrative`から共有helper参照を除去。各々該当testが失敗し、復元後に35件全passへ戻ることを確認した。
- 変更はtest 1ファイル（`03_Implement/frontend/src/api/client.test.ts`）のみで、production codeとrequest挙動は無変更である。single-tenant profileでheaderを送らない既存挙動、SafeMode既定ON、proposal-only、human-only `human_reviewed`、`SUI_LLM_PROVIDER=none`、fail-closed tenant checkはいずれも変更していない。
- 環境制約: backendを変更していないためbackend testとRuffは実行していない。SaaS専用Playwright（`playwright.saas.config.ts`）は検証コピーにbrowser binaryを用意していないため未実行である。残る未充足は従来どおり外部依存側であり、実trusted auth edge、実外部PDP／binding／capability serviceへの到達性、それらを前提とする全consumer越境matrixは未完了のため、AC-4/6/7/8/9/10/12/13は未チェックのままとし、`saas-multitenant`のsettings起動拒否も維持する。

### Implementation checkpoint 2026-08-06: multi-tab AI mutation browser confirmation

- 2026-08-02／08-03チェックポイントの多タブbrowser matrixに、AI mutation 1件を追加した。対象は`POST /ai/generate-narrative`（`api/client.ts`の`generateNarrative`、`App.tsx`の`handleGenerateNarrativeFromReadingOrder`）である。提示された`POST /ai/proposals/island-summary`ではなく本requestを選んだ理由は、islandの作成・選択という追加前提を必要とせず、既存の`e2e/helpers/i18n.ts`共有helper（`openAdvancedWorkMode`／`selectWorkModeTab`、`ce3_patch_workspace.spec.ts`が既に使用）だけでAdvanced UI有効化とNarrativeタブ表示まで到達できるため、本spec fileの最小document fixture（card 1件・island 0件）と既存5 testの割り込み方式（`page.route`でresponseを保留し、`releaseDelayedResponse`相当のclosureで後から解放する、test 1と同型）に最も素直に合致したためである。成功時は`document.narratives`へAI生成textを直接追加する（`applyDocumentChange`経由、`reviewed: false`）ため、内容を保持したまま適用されてはならないproposalとして代表性を持つ。
- 追加testは次を行う: tabAでAdvanced UIとNarrativeタブを開き、`**/api/ai/generate-narrative`を`page.route`で捕捉して応答を保留したまま「Generate from Reading Order」を押す。`releaseDelayedNarrative`が定義されるまでpollしてrequestがin-flightであることを確認してから、tabBでtenant Bへ切替える。既存5 testと同じ`BroadcastChannel`coherence機構によりtabAが即時blocked viewへ遷移することを確認し、保留していた200応答（本文`"tenant-a confidential narrative draft"`）をその後で解放する。解放後もblocked viewが維持され、この文字列がtabAのDOMに一度も出現しないこと、およびtabBのDOMにも漏れていないことをassertする。tabBは切替後に自身のtenant-B文書（`tenant-b confidential card`）を独立して表示し続けることも確認しており、これが「新規タブ以降はtenant-Bスコープのrequestだけが信頼される」ことの証拠である。証明している内容は、2026-07-20チェックポイント「stale network response generation guard」がunit testで既に固定していたtenant session generation guardの適用範囲（Document read/write、export監査、AI mutation）のうち、AI mutation側についても他5 testと同じ実ブラウザ・実`BroadcastChannel`条件下で成立することであり、新しい保護機構の追加ではない。
- 検証: `~/kjnative-fe`へ変更ファイル1件（`e2e/tenant_session_multitab.spec.ts`）のみ`rsync`した。`npx tsc --noEmit` pass。実browserでの`SUI_E2E_SAAS=1 npx playwright test --config=playwright.saas.config.ts`は今回**実行できた**（検証コピーに`~/.cache/ms-playwright`のChromiumがあることを確認済み）: 既存5件＋新規1件の計6件全passを実際のブラウザ実行で確認した（新規testも含めた実測、モック不使用のfile読み替えではない）。デフォルト設定（`SUI_E2E_SAAS`未設定）での`npx playwright test e2e/tenant_session_multitab.spec.ts`は新規testを含む6件全てが引き続きskip判定であることも確認した。`npm run test`はfrontend全体1,381件中1,380件pass・232 file中230 file pass（失敗2 fileは2026-08-06先行チェックポイント既述のとおり検証コピーが`02_Architecture/`・`04_Documentation/`を持たない環境起因で、本変更前から同数）。`npm run build` pass。WSL-nativeスナップショット（gitdirがWindows絶対pathのため`~/kjnative-fe`ではなく`/tmp`への一時snapshotから実行）で`python3 01_Plans/docs_check.py` pass（`active_memos=57, tracked_markdown=477`、本checkpoint追記後の値）、`python3 01_Plans/issues/validate_active_issue_memos.py` pass（57 active issue memos）。Windows側`git diff --check`もpass。
...
- 変更はtest 1ファイル（`03_Implement/frontend/e2e/tenant_session_multitab.spec.ts`）のみで、production code、backend、他のfrontend testは無変更である。SafeMode既定ON、proposal-only、human-only `human_reviewed`、`SUI_LLM_PROVIDER=none`、fail-closed tenant check、既存5 testの契約はいずれも変更していない。
...
- 同時に、呼ばれた名前を`route.endpoint.__globals__`で実オブジェクトへ解決し、対象オブジェクトとの同一性で比較する方式に変えた。これにより`from sui_sensemaking_api.db import get_db as _session_factory`のような別名importが、文字列一致の検査（旧`"get_db" not in ...`）を回避しつつ実際にはDBへ到達する、という迂回を防ぐ。`_authorize_request`／`_authorize_document_policy_management`／`resolve_trusted_saas_request_session`／`require_current_tenant_session_version`を実オブジェクトとしてimportし、比較対象をこれらの同一性チェックへ統一した。
...
### Implementation checkpoint 2026-08-06: request-time adapter-missing deny at the Document enforcement point

- AC-4が列挙する5条件のうち、enforcement点にlocalなdeny分岐を持たないものが1つだけ残っていることを、記述の再掲ではなく現在のコードで確認した。tenant不明・tenant不一致は`access_control.py`の`apply_tenant_boundary_guard`が`tenant_context_missing`／`resource_tenant_missing`／`tenant_mismatch`をPDP呼出し前に返し、membership停止は`tenant_context.py`の`_deny_inactive_membership`（`403 tenant_membership_inactive`）と`recheck_trusted_tenant_context`が`saas_request_context.py`経由で拒否し、PDP不達は`resolve_access_decision`が`policy_ref_unreachable`へ正規化してSaaS必須の`deny` fail-safeでreadも拒否する。いずれも認可経路そのものの中に直接のdeny分岐がある。
- **adapter欠損だけが例外だった**。`routes/docs.py`の`_authorize_request`は`request.app.state.access_control_adapter`が`None`のとき、runtime profileを問わずtenant境界guardだけを適用して`AccessDecision(allow=True)`を返していた。稼働中のSaaSでこの分岐へ到達しないのは、(a) `trusted_saas_runtime.py`の起動preflightが`saas-multitenant`に`access_control_adapter=external_http`と実`ExternalPolicyAccessControlAdapter`を要求して起動自体を止めること、(b) `main.py`のlifespanが必ず`build_access_control_adapter()`の戻り値を`app.state`へ設定し、当該factoryが（`settings.py`が受理する`noop|mock|external_http`のいずれでも）`None`を返さないこと、という**2つの離れたファイルの挙動**にのみ依存していた。契約がenforcement点からは読み取れないため、どちらかがrefactorされてもlocalには何も落ちない。lifespanを起動しない素の`TestClient(app)`（`test_adapter_unset_keeps_existing_docs_roundtrip`）では現に到達する分岐である。
- `_authorize_request`のadapter欠損分岐へ、tenant-scoped profileのときだけ`AccessDecision(allow=False, reason="adapter_missing")`を`enforce_access`へ渡す直接のdenyを追加した。profile判定はfunction冒頭で1度だけ評価した既存の`tenant_session_precondition_required(request)`の結果を再利用し、profile検出の第2経路を作っていない。`enforce_access`は`allow=False`をaction非依存で拒否するためreadも含めて`403 Access denied: adapter_missing`となり、`_read_only_fallback`のread許容経路を通らない。既存のtenant境界guardの**後ろ**に置いたため、tenant不明・不一致が同時に成立する場合の理由コードと優先順（`resolve_access_decision`と同じくtenant境界が先）は変えていない。
- 理由コードは`access_control.py`の`FailSafeReason`へ`adapter_missing`として追加した。同列挙の既存adapter系が`adapter_error`という短い接頭辞であるため、`api.md` §8.3の「adapter例外/想定外応答: `adapter_error`」と対になる名前にした。access-control理由コードの命名規約文書と、これに近い既存名は探した範囲では存在しない。`api.md` §8.3のfail-safeマトリクスは`SUI_ACCESS_CONTROL_FAIL_SAFE_MODE`に従って倒れる4系統の表であり、本条件はmodeに関係なく常にdenyするため同表へは追加していない（既存のtenant境界3理由も同表に無く、扱いを揃えた）。
- **正しく設定された稼働中のSaaS deploymentで許可される操作は変えていない**。上記(a)(b)が両立する限り追加分岐は到達不能であり、今回入れたのは同じ保証をenforcement点へ直接書き下したdefense-in-depthである。`ADR-0059`は既に「adapter欠損、`noop`、PDP timeout、無効応答、tenant解決不能では**起動またはrequestを**fail-closedにする」と決めており、今回はそのrequest側を明示しただけであるため新規ADRは追加していない。single-tenant（`local-dev`／`evaluation`／`enterprise-production`）でのadapter未設定Document roundtrip許可、`access_control_fail_safe_mode`の扱い、外部PDPの評価内容、SafeMode既定ON、proposal-only、`SUI_LLM_PROVIDER=none`はいずれも無変更である。auth edge／identity解決層（`SAAS-TENANT-AUTHEDGE-01`所管）にも触れていない。
- 同型のgapが他に無いことも確認した。`app.state.access_control_adapter`をenforcementのために読むのは`_authorize_request`の1か所だけで、残り3か所は監査metadataの`adapterName`取得である。Tenant Admin側の`_authorize_document_policy_management`はPDP adapterではなくcapability resolver経路であり、resolver欠損時は既に503で閉じる。
- 検証: `test_docs_access_control_integration.py`へ`test_tenant_scoped_profile_denies_read_when_adapter_is_missing`を追加した。隣接する`test_adapter_unset_keeps_existing_docs_roundtrip`と同じ「adapter未設定・自tenantに実在するDocument」という条件を保ったままruntime profileだけを`saas-multitenant`へ切り替え、readが403 `adapter_missing`となり、Documentのtitleとcard本文がresponseへ出ないこと、同条件のwriteも403となることを固定した。資源を実在させているため、このdenyが404やtenant境界理由の取り違えでないことを示せる。
- 契約が実際にfail-closedであることを2 mutationで確認した: (1) 追加したdeny分岐を無効化すると同一requestが403ではなく**200になりDocument取得が成立する**（これが今回塞いだ差分そのものである）、(2) profile条件を外して無条件denyにすると`test_adapter_unset_keeps_existing_docs_roundtrip`が403で失敗する（single-tenant互換が実際にtestで守られていることの確認）。各mutation後に対象fileがbyte一致で復元されることをhashで確認し、全testがpassへ戻ることも確認した。
- 検証結果: `test_tenant_session_precondition.py`／`test_docs_access_control_integration.py`／`test_access_control_tenant_boundary.py` 32件pass（変更前31件＋新規1件）。2026-08-06先行チェックポイントのregistered-route coverage契約は`_authorize_request`をobject同一性で参照しendpoint本体だけを走査するため、今回の関数内部の変更で判定は変わらず、exemption表も追加していない。backend全体648 passed・条件付き25件skip・failed 0（Windows側venv `03_Implement/backend/.venv`、Python 3.14.5）。`ruff check .` pass、`git diff --check` pass、docs-check pass（active_memos=58、tracked_markdown=473）、active issue validator 58 memos pass。
- 全体suiteは`.venv/Scripts`をPATHへ前置して実行する必要がある。前置しない起動では`subprocess.run(["alembic", ...])`でmigrationを呼ぶtest群15件が`alembic`実行体を解決できず失敗する（2026-07-29チェックポイント既述の同じ問題のWindows版）。回帰の有無はこの条件でも別途確認した: 変更後633 passed・15 failedに対し、未変更のmain checkout（`C:\GIT\sui-sensemaking`）が**同一の15件を同一test名で失敗させ632 passed**となり、差はnew testの1件だけで新規失敗は無い。
...
### Implementation checkpoint 2026-08-13: AC-8/AC-9を監査で充足、AC-10を強化、AC-6/13チェック欄の訂正

前回までの残ACのうち、どれが実際に着手可能かをまず切り分けた。AC-4は実trusted auth edge、AC-7は`ADR-0072`（Proposed、「採択前に実装へ着手しない」）、AC-6/13は本ページ2026-08-11監査の`issue-SAAS-TENANT-SESSION-BINDING-01`（`ADR-0074` Proposed待ち）にそれぞれ依存し、いずれも保守者判断か外部環境が前提のため今回は着手しなかった。AC-8/9/10は依存ADRを持たず、現在のコードに対する監査と欠落の是正だけで前進できると判断し、これを対象にした。

**AC-9（export/importのtenant境界）**: `grep -rn "@router\.(get|post|put|delete|patch)" routes/` で31件のroute定義を再列挙し、`frontend/src/api/client.ts`の`fetch`呼び出し（18箇所）と`App.tsx`の`fetch`呼び出し（3箇所、いずれも`./packs/...`というfrontend originの同梱public pack読込でbackend APIではない）も独立して再列挙した。2026-08-06チェックポイントが記録した「server側import/share endpointは存在しない」という結論と一致し、`/import`・`/share`相当のroute、frontendからの該当requestのいずれも見つからなかった。exportに相当する唯一のserver接点`POST /docs/{doc_id}/export-audit`は監査記録専用でDocument本文を移送せず、`_authorize_request`と`require_tenant_scoped_api_precondition`の対象である。importはbrowser内のFile読込・JSZip展開・previewだけで完結し、永続化は既にtenant-scoped・session-precondition-guardedな`PUT /docs/{doc_id}`だけを経由するため、サーバは常にactive TenantContextで再認可する（client由来のtenant主張を信用しない）。人手レビューは既存の`human_reviewed`・明示適用操作が担っており、tenancy追加以前から成立している。以上によりAC-9は現状で充足していると判断した。

**AC-8（cache/job/MCP/agent credential/audit/storage keyへのtenantId伝播）**: Explore agentへ、既に確認済みの項目（localStorageのtenant scope facade、JWKS cacheのprovider単位key、tenant必須audit event envelope、非同期job機構が存在しないこと、`saas-multitenant`でMCPが起動しないこと）を除いた「他に見落としているcache/storage機構」の横断調査を委託し、file:line根拠付きで8件の候補を得た。このうち7件は問題なしと確認した: representative visual cue画像のIndexedDB（`domain/representative_visual_cue_assets.ts`）とagent task ledger（`storage/agent_task_ledger.ts`）は`scope`が全呼び出し箇所で実際に渡されている、OAuth PKCE verifierの`sessionStorage`は認証前でtenant概念自体が無くtabスコープ、backendのrate limiterはIPキーでtenantデータを保持しない、CE4監査tracker（`routes/docs.py`）は`tenant_id`が既にkeyの一部、`InMemoryActiveTenantSessionPersister`（`active_tenant_session.py`）は`saas-multitenant`で`main.py`から構築されず到達不能、object-storage adapter（`external_content_store.py`）は`tenant_id`欠落で例外を投げる設計だが現状未配線。**残り1件は実際の欠落だった**: `ui/AppErrorBoundary.tsx`のcrash-recovery document snapshotが固定key `sui-sensemaking/evicted-doc`をscope引数なしで使っており、他の全storage helperと異なりtenant/principalを区別しない。`App.tsx`は`loadEvictedDocument()`を全mountで無条件に呼び出し、復旧banner表示にtenant確認が無いため、同一ブラウザでtenant Aがcrash→tenant Bへactive tenant切替という経路で、tenant Bにtenant Aの文書全文が復旧提案される。`buildTenantStorageKey`（`storage/tenant_scope.ts`、既存の`agent_task_ledger.ts`と同じ形）を使うようscope引数を追加し、`App.tsx`の3箇所の呼び出し（`pendingRecovery`初期化、`applyRecoveredDocument`、`handleDismissRecovery`）と`AppErrorBoundary`のJSX使用箇所へ`appStorage.scope`を配線した。単一テナント（`scope`未指定）では従来どおり無scope keyへfallbackし後方互換を保つ。tenant/principalが別々のkey名前空間へ入ることで、既存のtenant切替cleanup（`session/tenant_transition.ts`の`clearTenantScopedStorage`、prefix一致で削除）の対象にも自動的に入るようになった。AC-8の各要素を現在のコードで確認し1件の実欠落を是正できたため、充足したと判断した。

**AC-10（同一docIdの越境negative matrix: API/MCP/worker/browser cache）**: 既存の`test_all_rls_protected_document_tables_fail_closed_across_pool_reuse`はtenant A/Bへ異なる`doc_id`（`doc-a-*`/`doc-b-*`）を割り当てており、全queryが`tenant_id`条件を併記しているため、「同じdocIdを作成した」というAC-10の文言が要求する状況（RLSが無ければ`id`だけのqueryが両tenantの行を返す状況）を再現していなかった。同ファイルへ`test_identical_doc_id_across_tenants_stays_isolated_under_rls`を追加し、tenant A/Bへ文字列として同一の`doc_id`を持つ4表の行を作り、**`tenant_id`を一切含まないid-only query/update**（`documents`/`merge_decision_logs`/`document_access_metadata`/`document_access_admin_audit_events`）が常に呼び出し元自身のtenantの1行だけを返し・更新することを固定した。テストの有効性は2通りの変異で確認した: (1) `sui_sensemaking_runtime`と同じ非superuser roleだが`BYPASSRLS`を付与したroleへ切り替えると、fixture自身の`rolbypassrls is False`事前chekcで即座に検出・停止する。(2) 対象4表の`ROW LEVEL SECURITY`を一時的に`DISABLE`すると、新テストは1周目の`documents`select assertionで想定どおり失敗し（tenant Bの行が漏れて2件返る）、RLSを再度`ENABLE`+`FORCE`すると5件全passに戻ることを確認した。API層はこれで「異なるID」「同一ID」の両方をRLSで固定できたが、AC-10が要求するMCP/worker/browser cache次元は既存資産（MCP起動時fail-fast、worker単位のgeneration guard unit test群、`tenant_session_multitab.spec.ts`の実ブラウザ多タブ matrix）に留まり、7種のAI mutationのうち実ブラウザ多タブで確認済みなのは1種のみという2026-08-06の訂正記録が示す残課題（`issue-SAAS-TENANT-E2E-01`所管）は解消していない。したがってAC-10は新しい一次元を固定できたが、4次元の完全な網羅は主張せず未チェックのまま残す。

**AC-6/AC-13チェック欄の訂正**: 上記2026-08-11監査（本ファイル直前のセクション）はAC-6/13を「未充足」と明記していたが、AC-6のチェック欄自体は`[x]`のまま、AC-13の`[~]`注記にはこの前提欠陥への言及が無く、チェックリストが監査結果と食い違ったままになっていた。新しい調査ではなく、既に記録済みの2026-08-11の結論をチェック欄へ反映しただけである: AC-6を`[~]`へ変更し`issue-SAAS-TENANT-SESSION-BINDING-01`への参照を追記、AC-13の注記にも同issueへの参照を追加した。

**検証**:
- backend: 変更は`tests/test_document_access_rls_postgres.py`のみ（production codeは無変更）。`ruff check tests/test_document_access_rls_postgres.py` / `ruff format --check` 両方pass。全体`pytest`は992 passed・34 skipped（`postgres` marker）・8 deselected（`external_llm`）・0 failed（Windows側`.venv`ではなくWSL native venvで実行、Python 3.10.12、alembicはPATH前置で解決）。
- PostgreSQL実地検証: WSL Docker（v28.3.3）で使い捨て`postgres:16-alpine`コンテナ（ホストport 15432）を起動し、migration owner役の`postgres`superuserと、superuser属性・`BYPASSRLS`を持たない`sui_sensemaking_runtime`roleを分離して作成、`alembic upgrade head`（25 revisions）を適用した。`SUI_RUN_PG_RLS_TESTS=1`で`tests/test_document_access_rls_postgres.py`5件（新規1件を含む）が全passし、上記の2種の変異検証も実施した。検証専用コンテナは本チェックポイント記録後に削除する。
