...
| 対象 | 現在の不一致 |
|---|---|
| `ContextQueryV1` | `02_Architecture/architecture.html` は `scope:string[]` / depth列挙 / SafeMode object、`schemas.md` は `queryId` / scope列挙 / depth数値 / `safeModePolicy:"strict"`。 |
...
| 文書構造 | `02_Architecture/architecture.html`、`api.md`、`schemas.md`、data model overviewに日付付きfreeze、Stream実行ログ、checkpoint、reaffirmationが残り、章番号も前後する。 |
...
- `02_Architecture/architecture.html`: コンポーネント、責務、信頼境界だけを正本化し、型シグネチャを再定義しない。
...
- `02_Architecture/data_model_operations_overview.html`: 現行の物理モデル、CRUD、support level、運用責任だけを保持する。
...
- [x] `02_Architecture/architecture.html` は責務、`schemas.md` は型、`api.md` はendpointという責務分離を守り、他文書はanchor参照だけを持つ。
...
- [x] T4 `02_Architecture/architecture.html` / `api.md` / data model overviewを責務別に縮約する。
...
- 実行コマンド:
  ...
  - `rg -n "holdState|shelf|meta|contradictionSignalDecisions|ka" 02_Architecture/schemas.md 02_Architecture/data_model_operations_overview.html`
  - `rg -n "^#{1,4} .*?(Stream|freeze|rerun|execution log|checkpoint|reaffirmation)" 02_Architecture/architecture.html 02_Architecture/api.md 02_Architecture/schemas.md 02_Architecture/data_model_operations_overview.html`
...
- T5 partial (H-D): `02_Architecture/data_model_operations_overview.html` former §1.2/§1.3/§8〜§13を`history/data-model-operations-stream-d-2026-05.md`へ移し、元文書・履歴索引・reading guideの双方向導線を追加した。
...
- T4 partial / T5 H-A: `02_Architecture/architecture.html`のCE0 snapshot、旧§7A.2.1型・method・event-order再掲、2026-05-04 baseline、Stream B反映メモを`history/architecture-contract-freeze-formation-2026-04-to-05.md`へ物理移動した。
...
- 検証: 移動元39行の履歴保持、変更文書の相対link 21件、`02_Architecture/architecture.html`の履歴風見出し0件、Active issue validator 33件、validator/triage unit 11件、CE1 backend contract 18件、CE1 frontend contract 9件がpassした。
...
- 現行§2.8はendpoint/status/error/副作用に縮約し、型・キー・canonicalization・version互換を`schemas.md`、責務・信頼境界を`02_Architecture/architecture.html`、未解決`queryId`/`schemaVersion` envelope差異を`CE1-CONTRACT-01`へ参照する。CE4 mock/stub境界は現行§2.9.5へ移設し、意味を変更していない。
...
- T4: `02_Architecture/architecture.html`の重複§7A.6を除去し、§7BをCE1/CE2/CE4の責務・信頼境界と正本リンクへ縮約した。required key、列挙、error、HTTP副作用の再掲をやめ、型=`schemas.md`、HTTP=`api.md`、責務=`02_Architecture/architecture.html`へ一意化した。
