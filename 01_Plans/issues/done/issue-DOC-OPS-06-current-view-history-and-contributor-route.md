...
## Follow-up 2026-08-07: 実行コマンドの静的照合

文書内の実行コマンドを現行契約（endpoint・CLI option・service・package script）へ静的照合する横断チェックは `issue-DX-DOC-04-executable-command-contract-checks.md` を正本とする。本issueはcurrent-view履歴・貢献者routeの整理に限定し、実行コマンドの整合はDX-DOC-04へ委譲する。
