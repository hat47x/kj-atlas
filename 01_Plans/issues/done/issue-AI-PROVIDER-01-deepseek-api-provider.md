...
- Priority: P1
- Owner: Maintainer
- Scope: `03_Implement/backend/src/sui_sensemaking_api/llm/provider.py`, `03_Implement/backend/src/sui_sensemaking_api/settings.py`
...
- 現在の問題: sui-sensemakingのLLM providerは既存のモデル（Claude等）のみに対応しており、安価なDeepSeek APIをKJ操作の大部分に使うことができない
...
- 実施すること:
  ...
  2. 環境変数 `SUI_DEEPSEEK_API_KEY` / `SUI_DEEPSEEK_BASE_URL` / `SUI_DEEPSEEK_MODEL` で設定可能にする
...
- [x] `SUI_LLM_PROVIDER=deepseek` でDeepSeek APIが使用可能 — provider.py DeepSeekProvider実装
...
- `SUI_DEEPSEEK_BASE_URL` の既定値は `https://api.deepseek.com`
