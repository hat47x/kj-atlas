...
- Status: Done
- Source Issue: `01_Plans/research/direction-review-2026-08-13.md` 優先3-3
- Priority: P2
- Owner: Maintainer
- Scope: `03_Implement/backend/src/sui_sensemaking_api/routes/ai.py`（`_build_island_summary_prompt`）, `03_Implement/frontend/src/ui/SidePanel.tsx`, `src/i18n/locales/*.json`
- Related ADR/Spec: `00_Prompt/sensemaking_technique.md`（§3 表札検査・戻し検査）, `00_Prompt/ai_sensemaking_execution_procedures.md`（§3 表札検査を自己実行）, `01_Plans/adr/ADR-0067-three-element-constraint-design-method.md`
- Norms: `SUI-INSPECT-02`, `SUI-SIGN-12`, `DOM-CORE-02`, `DOM-CRIT-03, DOM-CRIT-04, DOM-CRIT-07`（`not_the_same`/`feels_off` タグを再提案プロンプトの制約条件として明示する実装）
- Expected verification level: `integration`

## 課題

方法論の表札検査は「書いた表札を島の各カードへ戻し、『あなたが言いたかったのはこれか』と照合する。1枚でも『ちがう』が返ってくるカードがあれば書き直す」（`sensemaking_technique.md` §3）。

> `SUI-INSPECT-02` 戻し検査（`00_Prompt/sensemaking_technique.md` §3）／`00_Prompt/ai_sensemaking_execution_procedures.md` §3 表札作成（島ラベル）
>
> 未実装の帰結として `SUI-SIGN-12`（どのカードからも「ちがう」が返ってこなかった）が検出不能である。
