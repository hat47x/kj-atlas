...
- Related ADR/Spec: `00_Prompt/sensemaking_technique.md` §4（島間関係線）, `02_Architecture/api.md`（summarize-island-relation 契約）, `01_Plans/issues/done/issue-DOGFOOD-15-island-relation-summary-grounding-always-empty.md`（接地カードの保全）, `01_Plans/issues/done/issue-DOGFOOD-21-narrative-text-not-grounded-in-reading-order.md`（テキスト接地の同型）
