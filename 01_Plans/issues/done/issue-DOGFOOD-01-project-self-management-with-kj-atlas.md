# Issue: DOGFOOD-01 sui-sensemaking によるプロジェクト自己管理の開始
...
- Priority: P1
- Owner: Maintainer
- Scope: `01_Plans/`, `02_Architecture/`, 新規 sui-sensemaking 文書
...
- 実施すること:
  1. 本プロジェクトのADR・issue・設計文書・制約をカード化し、sui-sensemaking文書として管理する
  2. 最初の題材: 現在の未解決設計課題とその相互関係（non-canvas-ui-flow-design.html §9の表を出発点とする）
  3. W型累積KJ法（R1〜R6）をプロジェクト改善に適用する
  4. ドッグフーディングで発見された sui-sensemaking 自体の改善点を issue 化する
- 実施しないこと:
  1. 既存のissue管理（01_Plans/issues/）の置き換え（補完であり、移行ではない）
  2. sui-sensemaking文書のGit管理自動化（現段階では手動export/import）
...
- [ ] 最初の sui-sensemaking 文書（プロジェクト管理用）が作成され、R1のカード化が完了している
- [ ] ドッグフーディング中に発見された sui-sensemaking 自体の改善点が1件以上 issue 化されている
...
- ドッグフーディングの副次的効果として、sui-sensemakingのUI/UX改善点が発見されることを期待する
