...
- strict mode 例外運用の固定値（D1〜D4）は本Issueで再定義せず、`02_Architecture/strict_mode_exception_approval_flow.html` 正本参照に限定。
...
- Context: 役割分離、2者承認、strict 例外緩和、固定値 D1〜D4 は `02_Architecture/strict_mode_exception_approval_flow.html` の正本に収束済み。
