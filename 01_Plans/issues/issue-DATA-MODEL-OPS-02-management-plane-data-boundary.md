...
- Priority: P2
- Owner: Claude Code
- Scope: `03_Implement/backend/`（一覧API・エージェント登録）, `03_Implement/frontend/src/storage/`（recent/プリセットの正本格下げ）, `02_Architecture/design/data_model_operations_overview.html`, `02_Architecture/schemas.md`, `02_Architecture/api.md`
...
このOpen化は実装完了を意味しない。次の作業は `schemas.md` / `02_Architecture/design/data_model_operations_overview.html` / `api.md` の契約先行同期であり、それが完了するまで一覧API・登録API・管理UIを実装しない。
...
- [x] AC-1: D1〜D4 の判断が記録され、`02_Architecture/design/data_model_operations_overview.html` の ER図・CRUD表・サポートレベル表が同時更新される（§7 更新ルール遵守）。
...
- [x] AC-3: localStorage「最近」はサーバー一覧のキャッシュとして再定義され、両者の不一致時はサーバーを正とする。→ `02_Architecture/design/data_model_operations_overview.html` CRUD表 + `recent.ts` コメントとして契約反映済み（2026-07-17）。不一致時のサーバー優先ロジック実装は `GET /docs` API実装時に追加する。
...
- [x] AC-6: 管理UI設計要求（Round 8）の入力パッケージ（確定した正本・権限・本文非表示原則・対象画面一覧）が `ui_design_handoff.md` の受け渡し形式で準備できる状態になる。→ `ui_design_handoff.md`「正本と役割」表にD1〜D4の正本（`02_Architecture/design/data_model_operations_overview.html` §4・§5.2、`schemas.md` §3.4.1、`api.md` §2.4・§9.5）を指す行を追加し、既存の受け渡しパッケージ手順（画面一覧=`ui_catalog.md`、対象issue/ADR anchor参照）から辿れる状態にした（2026-07-20）。

### AC-1 実装証跡（2026-07-16）

- `02_Architecture/design/data_model_operations_overview.html` §2（物理ER図）: D1〜D3がいずれも新規物理テーブルを追加しない決定であることを説明する段落を追加した（文書一覧=既存`documents`の射影、View/Perspective・QueryPreset=既存`view.json`とdevice-local維持、エージェント登録=`EXT-CONN-02`実装まで契約先行でER図に含めない）。
...
- `02_Architecture/design/data_model_operations_overview.html`のCRUD表（§4、AC-1で記録済み）は既に同じ契約（`id`/`title`/`updatedAt`のallowlist、fail-closed方針）を記述していたため、今回の契約固定と整合していることを確認した（変更なし）。
...
- `02_Architecture/design/data_model_operations_overview.html`のCRUD表（§4、行167）を更新し、Create/Read/Delete列に上記3エンドポイントを反映、備考に`api.md` §9.5への参照を追加した。ER図（§2、既存記述）はEXT-CONN-02実装まで本テーブルを含めない方針を維持しており変更不要だった。
...
- Related: `02_Architecture/design/data_model_operations_overview.html`, `01_Plans/adr/ADR-0033-mvp-data-support-and-maintenance-boundary.md`, `01_Plans/adr/ADR-0035-privileged-data-lifecycle-boundary.md`, `01_Plans/adr/ADR-0054-external-connection-layer-staged-introduction.md`
