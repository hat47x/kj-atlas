...
- [ ] T1b 小さな島: メンバー数が閾値以下の島を導出し、島の表札へ「保護」を表示する。
- [ ] T1c 単独違和感: ドキュメント内で単独の違和感タグを導出し、スコア化せずに保護対象として表示する。
...
- ローカル環境では Playwright の Chromium ランタイムが不足しているため、実装時は `playwright test --list`、TypeScript、Vitest で静的・単体回帰を確認し、ブラウザ実行はランタイム導入後または CI で確認する。
...
- 凡例へ「保護」グループを追加した。T3 は View トグル未実装のため未完のままとする。
- 未完: 小さな島の表札表示（T1b）、単独違和感の保護表示（T1c）、View トグル（T3）、ブラウザ実行を含む e2e（T4）。
