...
- [ ] AC-1: `realistic_user_journey_expansion.spec.ts` が現在の共有パネルIA（目的選択ステップ経由）で完走する。
- [ ] AC-2: `ce3_patch_workspace.spec.ts` が現在のワークスペースIAで「候補を収集」に到達し完走する。
- [ ] AC-3: `npm run e2e:mock` が main 上で green になる。
- [ ] AC-4: UI文言・フローに依存するspecは `e2e/helpers/i18n.ts` の共有定数・ヘルパーを経由する（直書き禁止の徹底）。
