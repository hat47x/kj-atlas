...
上記「どちらの読みが正しいか」を、本issue初稿では自分自身の要約（`issue-DOMAIN-VISUAL-CUE-01`側の引用）からしか検討していなかった。`02_Architecture/design/sui-sensemaking 拡張提案.dc.html`のP37原文セクション全体（「原則4」だけでなく、問題提起の文・Before/After比較・原則1〜3）を直接読み直したところ、(a)側の読みを支持する技術的根拠がより明確に見つかった。要約ではなく判定そのものはまだ行っていない。
...
- [ ] P37の「自由絵文字入力は廃止する」が、(a) ADR-0060/T5のキュレート済み固定セット案も含めて絵文字という表現形式全体を廃止するのか、(b) 「自由入力」という入力方式だけを禁止し、キュレート済み固定セットなら許容されるのか、を明示的に判定する。→ 上記実装記録の技術的根拠は(a)を支持する。ADR改訂ドラフトを下記「判定補助資料」に用意済み（2026-08-07）で、判定者の承認待ちのみ。ADR改訂を伴うため判定自体は未実施。
...
## 判定補助資料: ADR-0060改訂ドラフト（(a)採用時の承認候補、2026-08-07）

下記は(a)（絵文字形式自体の廃止）と判定した場合にADR-0060へ適用する改訂案である。判定者が承認すれば、そのまま改訂PRとして提出できる。判定が(b)（キュレート済み固定セット許容）ならこの案は破棄する。

**ADR-0060 変更点（(a)採用時）**:

1. 供給経路の定義（§提案する決定 項目2）: 「B: Unicode絵文字/同梱プリセット」を「B: キュレート済みslate線画ピクトグラムセット（単色・1.5px・currentColor=slate-500）/同梱プリセットSVG」へ変更し、Unicode絵文字を供給経路から除外する。理由はP37原則1（多色不可・単色線画統一）にUnicode絵文字グリフが構造的に抵触するため。
2. `kind` enum から `"emoji"` を廃止し、`"preset_svg"`（同梱線画セット）と `"hand_drawn"` / `"user_image"` のみを残す。既にテスト・contract・SafeMode層へ `"emoji"` が実装されているため、廃止は移行ステップ（validator許容停止 → テスト更新 → 型から除去）で実施し単純deleteはしない。
3. `schemas.md §19` の `RepresentativeVisualCueKind` から `"emoji"` を除去し、§19.4「T7後続小PR」を「キュレート済みslate線画ピクトグラムセット」として再定義する。
4. `inquiry_bundle_safe_mode.ts` のフィールドポリシーから `"emoji"` 関連を除去。
5. T7チェックリストの「絵文字/プリセット」を「キュレート済みslate線画ピクトグラムセット」へ書き換える（`DOMAIN-VISUAL-CUE-01` T7）。

**影響範囲**: `types.ts`、`validate.ts`、`validate_doc.ts`、`inquiry_bundle_safe_mode.ts`、`schemas.md §19`、`unicode_emoji_os_comparison.md`（T5成果を「非採用」注記へ更新）。実装済み `"emoji"` がテストへ焼き込まれているため、廃止はPR分割（契約除去→validator除去→テスト更新）で実施する。

**このドラフトはADR-0060を改訂する提案であり、判定者が承認するまで確定しない。**
