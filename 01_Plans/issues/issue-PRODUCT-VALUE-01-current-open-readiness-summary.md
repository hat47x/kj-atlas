...
- The source issue remains `Draft` because the first meaningful map gate has not been converted into a fixed evidence packet with owner, screenshots, and replayable acceptance data.
...
1. Define a deterministic first-value fixture that exercises the manual authoring path without an LLM.
2. Add or refresh a focused Playwright flow that verifies card creation, first grouping/focus creation, visible undecided state, SafeMode/import validation visibility, and transition to save/share preflight.
3. Capture the same flow as a user-facing screenshot/evidence packet and reference it from `PRODUCT-QA-01`.
4. Keep `PRODUCT-VALUE-01` in `Draft` until the evidence packet exists and Productization Program Owner / QA Lead can review it.
...
- [ ] AC-02: A deterministic first-value fixture is defined before implementation work starts.
