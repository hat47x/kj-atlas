...
- [x] `Point`型へのid付加の要否が決定される。— **現時点では付加しない**（2026-08-12、三要素検証）
- [ ] 付加する場合、既存のdocument.jsonスキーマとの互換性を確認する。

## 三要素検証（2026-08-12、L2として仮承認判断）

| 次元 | 検証 |
|------|------|
| 業務 | 実害はDOMノードの再マウント（focus等の一時的喪失）に留まり、状態の取り違えには至っていない |
| データ | `Point`型（`{x, y}`）へのid付加はdocumentスキーマ変更を伴い、既存document.jsonとの互換性・マイグレーションが必要 |
| 機能 | `key={index}`の不安定性は、将来ハンドルがローカル状態を持った時点で初めて実害になる |

**判断: 現時点では`Point`型にidを付加しない。** 実害が限定的であり、スキーマ変更のコストに見合わない。将来、頂点ハンドルがローカル状態（ドラッグ中の選択・hover等）を持つようになった時点で再評価する（その時は合成id付加か、編集セッション内の頂点マップ導入を検討）。
