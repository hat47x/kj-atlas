...
| Frontend unit/regression | bundled `node.exe .\node_modules\vitest\vitest.mjs run` | Pass: 160 files / 732 tests | G1 / G3 / G7 |
| Full frontend Playwright E2E | bundled `node.exe .\node_modules\playwright\cli.js test --reporter=line` with Vite already running on `127.0.0.1:4173` | Pass: 27 tests | G2 / G3 / G4 / G7 |
...
- Remaining follow-up: slow/backend-recovery evidence remains routed to `PRODUCT-UX-04` and `PRODUCT-OPS-01`.
...
| G4 逕ｻ髱｢閠先ｧ | Conditional Go | 390px/768px/920px/1280px/1440px header-panel fit, Share/View keyboard focus return, canvas hit-testing, and synthetic large-document operability are now covered; slow-environment matrix remains open. |
