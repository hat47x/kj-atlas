...
一方、今後の完了メモまで同じ場所へ増やさない。`validate_active_issue_memos.py` は58件を一時的なlegacy上限として扱い、件数の減少は許容するが増加はエラーにする。新たにDoneへ遷移するメモは、参照先を必要に応じて同時更新したうえで `done/` へ移す。

この58件という数はDoneメモの正規配置を意味しない。既存legacyを段階的に減らす間だけ使うラチェットであり、整理が進んだ時点で上限を引き下げるか、最終的に0へする。
