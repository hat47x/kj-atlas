...
> `kj_*`を含むRelatedのパスやworking branch名は既存の技術識別子であり、本issueで用いる技法の一般名称ではない。

## 三要素整合（ADR-0067）

- **業務設計（Business）**: 親和図法の束ねでは、語彙類似ではなく「訴えの近さ」を2〜3枚の小さな集合として感じ取り、人間が表札を書く前の探索を支える必要がある。本PoCは候補生成を検証し、自動島形成は行わない。
...
- FlyVec / Comply等は疎な生物模倣motifから言語意味表現まで進める可能性を示すが、親和図法で必要なlatent advocacy、hard negative、2〜3枚のset-level coherenceに届くかは未検証である。
- 「軽量だから」「ハエ脳由来だから」という理由自体は採用根拠にしない。

要求水準に届けば、provider=`none`でも常時localに候補関係・違和感の契機を浮上させ、LLMとは異なる認知経路を持てる可能性がある。届かなければFly-inspired方式は近傍index等の限定用途へ縮小し、深層意味候補にはlocal sentence encoderまたはLLMを使う。
...
2. 深層意味表現が必要ならFlyVec/Comply-inspired encoderに研究価値があるか。
3. 一般sentence encoderに対し、親和図作業固有のfeedback / graph / space / historyを加えることに独立増分があるか。
4. その増分が探索を広げるのか、機械候補へのanchoringを強めるのか。
...
2. benchmarkにはhard positive / hard negative / held-or-ambiguous / singleton islandを含める。
3. 各annotationは元カードID、source snapshot、判断時点へ戻れるようにする。
4. 最初のbaselineは次の3系に限定する。
   - A: character/word n-gram + TF-IDF cosine
   - C: FlyHash-like sparse expansion + WTA
   - E: strong local sentence encoder
5. A/C/Eの結果からD（learned sparse semantic encoder）を作る意味があるか判定する。
6. Dに進む場合もFlyVec/Complyを無批判に移植せず、日本語短文・親和図カードという入力条件を明示する。
7. F（affinity-specific multi-channel extension）はsemantic channel単体を評価してから追加する。
8. 評価は`COGNITIVE-EVAL-01`のM1/M3/M4/M7/M9へ接続する。
9. 全方式で同じsource snapshotと事前固定benchmarkを使う。
10. seed、algorithm version、parameter setを記録し、同一入力から候補を再生成できるようにする。

### 実施しないこと

- benchmark作成時にモデル候補を見てannotationを調整しない。
...
- [x] **T1 Research**: FlyHash / BioHash / FlyVec / Comply / APL局所抑制と親和図法の要求との差を整理する。
...
CまたはDがAに対してhard positiveを改善し、同時にR2/R3を壊していないことを見る。

### Gate B — Fly-inspired方式に独立した理由があるか

Eと比較し、品質・continuous-local負荷・更新容易性・説明可能性・affinity-feedbackへの適応の少なくとも一つに検証可能な利点があるかを見る。Eの方が単純で同等以上ならFly-inspired方式を意味層として採用しない。

### Gate C — 親和図作業固有情報に増分があるか

一般意味表現だけの場合とFを比較する。Fの改善がなければgraph/space/historyをsemantic kernelへ混ぜず、既存構造として別に保持する。
...
- データ境界: PoCはlocal/offlineを既定とし、dogfoodカードを外部providerへ送らない。
- SafeMode/監査/人間レビュー境界: proposal-only、人間確定、非序列化を維持する。

## 補足

本issueは新ADRを起票しない。`ADR-0047`のexecution-first方針に従い先に実証する。PoCの結果として新しい永続状態、production API、provider、安全境界、性能予算超過等が必要になった場合のみR-1..R-4を確認して設計判断へ戻る。
