...
- [ ] `DC-CMD-001`がendpoint、CLI option、npm script、Compose service、pathの各負例をrule ID付きで検出する。→ npm script・Compose service・path・CLI optionの4区分が実装済み。endpointの1区分のみ未実装（follow-up、下記「実装記録」参照）。
