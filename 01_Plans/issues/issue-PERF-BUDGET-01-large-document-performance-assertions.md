...
- [x] 代表規模 fixture で主要操作の性能アサーションが存在し、緑である。
- [x] メインスレッドを長時間ブロックする同期処理が無い（重処理は worker 経由）ことを検証する。
- [x] 大規模・低速時に待機/進捗表示が出る（PB-5、`ADR-0044` UQ-5 と整合）ことを確認する。
- [x] 閾値超過時にテストが赤になる（回帰検知）。
- [x] スキーマ・実行挙動に変更がない（テスト追加が主、必要時のみ debounce/メモ化の最小調整）。
