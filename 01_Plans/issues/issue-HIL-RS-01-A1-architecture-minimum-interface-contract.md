...
- Execution order (Stream A fixed serial): 1/4 HIL-RS-01 A1
...
- 想定との差分: 既存本文のOwner/allowlist表記がStream A前提のまま残存（本実行はStream F専任のため運用上の差分として明記）。
...
## Stream A serial approval lock（A1先行・Phase 1〜6厳守）
