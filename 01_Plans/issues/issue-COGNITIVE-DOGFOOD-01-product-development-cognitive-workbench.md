...
- 実施すること:
  ...
  2. KJ Atlas自身の未解決かつ開いた問題を新規dogfood題材にする。例: プロダクト位置づけ、第三者価値検証、AI責任境界、導入モデル、複雑なUI/情報設計、相反する要求の調停。
  3. GitHub上のADR/issue/コード/dogfood観察/外部リサーチを生材料としてカード化し、出典を保持する。
  4. 生成AIはカード化、探索質問、束ね候補、表札候補、反対視点、空白探索、B型叙述などをproposalとして支援するが、人間の確定操作を置き換えない。
  5. 既存の分類体系・ADR構造へ最初からカードを押し込まず、生カード→束ね→表札→配置→空白→追加探索の順序を守る。
  6. 最終成果をissue/ADR/実装候補へ変換した後、元カード・根拠・保留・異論へ戻れるかを確認する。
  7. 同じ問題を通常のAIチャット中心でも扱う比較ケースを設け、`COGNITIVE-EVAL-01` の指標で比較する。
- 実施しないこと:
...
- [ ] R1〜R5をケース0として監査し、既存試行の生存所見・消失所見・実際の後続変更を記録する。
- [ ] 少なくとも3件の新規実在開発課題で、認知dogfoodラウンドを完遂する。
