...
- AcceptanceScenario（前提 / 操作 / 期待結果 / 除外）: 前提=利用者が多数の島を含む文書を開いている / 操作=島を選び、絵文字・プリセット・権利確認済み外部素材・生成候補から任意の手掛かりを確認して採用する / 期待結果=表札と手掛かりを併用して島を見つけ直せ、画像なしへ戻せ、外部通信と権利条件を事前確認できる / 除外=自動分類、自動採用、画像だけの意味伝達、装飾目的の常設画像。
...
現状の設計は「画像生成（将来）」とだけ記載し、目的、対象、絵文字・プリセット・外部素材という代替経路、SafeMode、権利表示、アクセシビリティ、削除を定義していない。
...
- 供給経路を A: Unicode絵文字/同梱プリセット、B: 権利確認済み外部素材、C: proposal-only画像生成に分離する。
- Phase 1では通信不要の小規模な選択・解除・非表示を検証し、既定画面と既存操作を変えない。
...
- [ ] AC-6: Phase 0 fixtureで、画像なし・絵文字・プリセットの再発見時間、見誤り、主観的負担を比較している。
