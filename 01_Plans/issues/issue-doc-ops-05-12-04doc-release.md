...
- Context: `DOC-OPS-05-12` は DOC-OPS-05 の文書分類と公開品質を固定するためのDraft。
...
- Stream I との分離を維持するため、対象は **issue-doc-ops-05-12-04doc-release.md のみ** とし、他ストリーム専有ファイルは編集しない。
...
- 次実行単位（固定）: `04_Documentation/release.md` に公開可能なリリース責務分担と検証ゲートを明記する改善PRを起票する。
...
- Classification: **Improve external**
- 次実行単位は release の公開手順品質改善（docs-only）に限定する。

### Phase 4 Verify（docs-check）
- `rg -n "Audience|Goal|Non-goal|Public boundary|Outcome|Related|Go/No-Go|Phase 1|Phase 2|Phase 3|Phase 4|Phase 5" 01_Plans/issues/issue-doc-ops-05-12-04doc-release.md`
- `git diff --check`
...
- Read Order の上流（`00_Prompt/system_prompt.md` / `00_Prompt/domain.md` / `01_Plans/adr/ADR-0001-value-to-requirements.md`）と、対象Scope（`04doc_release.md`）の境界を突合。
...
- AC補強案: リリース手順の公開境界（Audience / Goal / Public boundary / Validation）を本文内で追跡可能に維持。
- DoD補強案: Proceed判定を `Ready / Hold / Needs-decision` の3値で固定し、Stopper該当時は `Hold` へ遷移。

### Phase 3) ADR CDC（必要時）
- 判定: **追加ADR不要**。
- CDC: Context=公開リリース手順の品質固定, Decision=Improve external分類維持, Consequences=後続は公開運用向け補強PRへ限定。
...
- 状態: **Ready**
- 次の1手: `04_Documentation/release.md` にタグ後検証と失敗時ロールバック手順を公開向けに補強するdocs-only PRを起票。
- 未解決点: リリース節でのCI失敗時エスカレーション導線（operations.md との責務分界）。
- Stopper確認: 未定義競合なし / safeMode後退語彙なし / 自己修復3回超過なし。

## DOC-OPS-05 Lane Update (2026-04-20)

### Phase 1) Read（対象Issueの現状・関連Spec確認）
- 対象: `issue-doc-ops-05-12-04doc-release.md`（Draft memoのみ）。
...
- 対象: `04_Documentation/release.md`
- Cross-reference確認: `04_Documentation/security.md` / `04_Documentation/operations.md` / `.github/workflows/release.yml`
...
- Execute outcome: release.md を公開release runbookとして維持し、security/operationsとの参照境界を明確化する。
...
- Issue固有Read: `Scope=04_Documentation/release.md` と `Related ADR/Spec`、`Requirement meta I/F` を再確認し、`VerificationLevel=docs-check` を固定。

### Phase 2: Plan
- 単一責務: `DOC-OPS-05-12` のIssueメモ品質を **Phase 1-5 直列処理** に正規化する。
- 実施計画:
...
- Scope再確認: `04_Documentation/release.md` の分類・公開境界の固定（docs-only）。
- Related再確認: `04_Documentation/release.md` / `01_Plans/documentation_quality.md` / `.github/workflows/release.yml`。
- DecisionStatus確認: `Fixed` を維持（分類再判定はしない）。

### Phase 2 Plan（AC/DoD不足提案の固定）
- AC追加固定:
  - `public-exposure` 観点で、公開可能なリリース手順と内部限定運用情報の切り分け基準を追跡可能にする。
  - `GoNoGoGate=Required` 判定根拠（Audience / Goal / 公開境界 / 次アクション）を本文で確認できる状態にする。
- DoD追加固定:
  - Proceed判定を `Ready / Hold / Needs-decision` の三値で明示する。
  - SafeMode既定ON・share/export漏えい防止の後退を許容しない。

### Phase 3 ADR CDC（必要時のみ）
- 判定: 追加ADRは **不要**（既存Issue内CDCで十分）。
- Context: 本Issueは公開リリース文書の分類品質固定であり、設計変更は扱わない。
- Decision: `Improve external` と `DecisionStatus=Fixed` を維持。
- Consequences: 後続のrelease文書改善PRで、公開境界と再現手順の同期基準が明確化される。
...
### Phase 5 Verify / Proceed
- docs-check: `python 01_Plans/issues/validate_active_issue_memos.py --files 01_Plans/issues/issue-doc-ops-05-12-04doc-release.md`
...
- 再読対象: `issue-doc-ops-05-12-04doc-release.md`。
...
- 関連正本（`01_Plans/documentation_quality.md` と `04_Documentation/release.md` の対応）を確認。
...
- 実施コマンド: `python 01_Plans/issues/validate_active_issue_memos.py --files 01_Plans/issues/issue-doc-ops-05-12-04doc-release.md`
...
### Phase 1 Read
- 対象Issue `DOC-OPS-05-12` の最新本文（Requirement meta I/F / AC / Validation plan）を再確認。
- Scope対象文書 `04_Documentation/release.md` を read-only 参照し、公開境界・読者・目的の現状を確認。
- 前提崩れ/競合検知: **なし**。
