...
- [x] AC-6: `04_Documentation/observability.md` を追加し、`operations.md` の「ログを見る」節と `04_Documentation/README.md` の公開一覧から参照した。**まだ観測できないこと**（メトリクス皆無、監査イベントが既定で捨てられること、ローカル保存と照会APIの不在、管理面が未監査、ログのローテーション未設定）も明記した。
