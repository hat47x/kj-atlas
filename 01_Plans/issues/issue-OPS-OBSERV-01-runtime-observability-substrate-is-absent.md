...
- **メトリクス基盤（`/metrics`）は未着手。** 本issueの「実施しないこと」に挙げたとおり、依存の追加と `ADR-0039`（個人OSSの運用規模）との整合が保守者判断を要する。**「入れない」も正当な判断であり、その場合は規模拡大を非目標として明示する必要がある**（`research/direction-review-2026-08-13.md` §5）。これが決まるまで「障害の検知は利用者からの申告のみ」という状態は変わらない。
