...
- Related Backlog: N/A
- Related ADR/Spec: `ADR-0001`, `ADR-0011`, `02_Architecture/architecture.html`, `02_Architecture/schemas.md`, `02_Architecture/enterprise_architecture.html`
- Dependencies: N/A
...
- `02_Architecture/enterprise_architecture.html` は組織要件（役割分離/監査）の根拠であり、責任分界点の要求定義に必須。
...
- [x] T4: 後続Issueテンプレへ責任分界点項目を反映する。→ `01_Plans/issues/TEMPLATE.md` に「責任分界（REQ-DEF-02）」節を追加（実行責任R / 受入判定A / 契約チェックポイント / 停止基準、任意）。
