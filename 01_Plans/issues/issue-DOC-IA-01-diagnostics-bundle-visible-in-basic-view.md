...
案Bを採用する。`ADR-0031` は診断を作業モード面の高度機能として定義しており、通常表示の認知負荷を増やさない方針とも一致する。ヘッダーの「サポート診断バンドル」と右側パネルの診断実行・診断追記・診断結果を「詳細」の内側へ移し、詳細を閉じた場合は開いていた診断バンドルも閉じる。文書の現行説明は正本ADRと一致しているため変更しない。
...
- [x] 選んだ案に従って文書または実装を変更し、両者が一致する。
- [x] 公開画像の再撮影（`DOC-SHOT-01`）より前に決着させる。
...
- `diagnostics_bundle.spec.ts` / `complexity_budget_foregrounding.spec.ts`: 通常表示で診断導線が存在せず、「詳細」を有効にすると操作できることを実ブラウザで確認。
- `header_toolbar_layout.spec.ts` を含む対象19 tests: 成功。
