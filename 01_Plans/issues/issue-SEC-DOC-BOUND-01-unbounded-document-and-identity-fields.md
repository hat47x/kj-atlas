...
- Priority: P3
- Owner: Maintainer
- Scope: `03_Implement/backend/src/sui_sensemaking_api/models.py`, `03_Implement/backend/src/sui_sensemaking_api/models_ai.py`, `03_Implement/backend/src/sui_sensemaking_api/routes/admin.py`
...
- **追記（2026-08-13・inventory 補完）**: `Card.sources`（`models.py:808`・`list[str] | None`）も無界（件数・1件あたり長さとも）。frontend `validate_doc.ts` も `validateStringArray`（文字列であることのみ）。provenance リストとして正当に増え得るが、巨大化で DB 行・LLM プロンプト（`_build_prompt` 等でカード情報に含まれる）を膨張させる。SEC-DOC-BOUND-01/02 の対象群に含めて上限判断の材料とする。
- **追記2（2026-08-13・実地確認）**: ドキュメント本文に**サイズ/カード件数上限が無い**ことを実証。約2MB・20,000カードの文書が `PUT /docs/{id}` で **200**（1.4s）・`GET` で 3.2MB 返却。backend に明示的なリクエスト本文上限が無く、カード件数無界のため DB 行を無制限に膨張させ得る。SEC-DOC-BOUND-01 の「カード/島の件数上限なし」の実効影響を裏付け。
