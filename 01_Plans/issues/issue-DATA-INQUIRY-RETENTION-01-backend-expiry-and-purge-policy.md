...
- [ ] AC-1: D1〜D3がMaintainerにより決定され、必要なら補足ADRがAcceptedになる。
- [ ] AC-2: 自動期限を採る場合、server-owned `expires_at`、policy version、状態、保持例外がportableなORM/migration契約として定義される。自動期限を採らない場合は、その非保証をUI・API・運用文書へ明記する。
