...
frozen commit上でこのblobが存在した座標は`01_Plans/issues/issue-AI-ROUTE-01-multi-model-routing-and-final-judgment-boundary.md`であり、current `done/` pathではない。したがって#3001の変更はcurrent lifecycle参照としては自然でも、frozen snapshot coordinateとしては不正だった。
