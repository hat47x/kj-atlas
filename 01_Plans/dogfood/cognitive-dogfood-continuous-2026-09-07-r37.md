...
### C. 設定可能性・required・recommended・conditionalも同じではない

#3050ではJIT=`true` 自体は不正値ではない。local-dev / evaluationでは意図的に使える。しかしEnterprise recommendationは、受理可能な値集合の説明ではなく、そのprofileで安全・標準として選ぶべき値のpolicy層である。

#3052は同じ混同を別方向から示した。local-devで `true` を使えることと、local-devを起動するために `true` が必須であることは違う。現行runtimeではJITは既定 `false` で起動でき、未登録header identityを自動作成するときだけ明示 `true` が必要だった。
...
4. **profile operating condition / policy** — required / recommended / conditional / optional のどれか。

を必要に応じて分ける。

### D. 「公開されている設定だから可変・必須」は成り立たない

公開keyが存在しても、現在の正本で意味が未確定なら可変にしておく必要はない。未定義behaviorへ値を流すdead controlは、設定可能性そのものが誤った約束になる。

逆に、実装済みconsumerがある設定を一律fixedへ縮めてもいけない。#3049のmock policyは実効制御として残し、その作用範囲だけを狭く明示した。JITもlocal/evaluation capabilityを残したまま、#3050ではEnterprise recommendationを `false` へ戻し、#3052ではlocal-devの `true` をrequiredからconditionalへ戻した。

## 3. 中心所見

**公開設定契約は「値を受理できる」で終わらない。値がどのruntime decisionへ結びつき、どこまで作用し、各profileでrequired / recommended / conditionalのどれとして扱われるかを分けて保持する。**
...
- R37: **effective control semantics** — 受理された値が実際に何へ効き、どう運用されるかを守る。

R37の中でもさらに、consumerへのbinding、作用範囲、profile operating condition / policyは別軸である。
...
4. 作用範囲が公開説明と一致する。
5. profile上のrequired / recommended / conditional / optionalがcapabilityと混同されていない。

## 5. Finding triage

- F0: #3048のdead toggle、#3049のconsumer scope過大表現、#3050のallowed valueとEnterprise recommendation混同、#3052のallowed local-dev capabilityとRequired settings混同という実陽性。
- F1: #3048のSettings fail-fast回帰、#3049の `test_ce4_mock_policy_scope_contract.py`、#3050/#3052の `test_jit_profile_recommendation_contract.py` へ所見を戻す。
...
またprofile policyをacceptance validatorへ逆輸入しない。EnterpriseはJIT=`false` 推奨でもlocal-dev / evaluationの明示 `true` capabilityは残るし、local-devで `true` が使えるからといって起動必須にもならない。
