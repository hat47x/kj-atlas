...
## 1. 現状サマリ（2026-08-16・iteration 208 時点）

| 指標 | 値 | 検証方法 |
|------|----|---------|
| 総シナリオ数（シナリオ1〜138） | 138 | §2.1 実走行＋`business-flow-e2e-scenarios-2026-08-15.md` |
| 業務フローE2Eスクリプトのシナリオヘッダ数 | 136 | verify script（シナリオ1=初期フロー・ヘッダechoなし / シナリオ4=別スクリプト`verify_admin_ops_flow_e2e.sh`） |
| カバー業態数 | 138 | シナリオドキュメント「カバレッジ集約」 |
| 業務フローE2Eチェック数 | 833 | §2.1 実走行 → `Result: 833 passed, 0 failed` |
| 総チェック数（check 10） | **889** | = 833（業務フロー）＋ 20（admin ops）＋ 7（kj multi-round）＋ 14（MCP CE-4 audit）＋ 15（CLI CE-4 audit）— 算術を verify script で照合 |
...
> 注記: 総チェック数は**業務フローE2Eのチェック数（833）とは別**。CIハーネス `verify_all.sh` check 10 は5自己完結E2E（業務フロー・admin ops・kj multi-round・MCP CE-4 audit・CLI CE-4 audit）を実行するため、総数 = 889。以前の進捗報告で総数が誤記されていた差分（-10）は、本マニフェストの算術照合により検出・訂正された。iteration 195 の実走行照合で、並行編集者が拡張した admin ops（12→20・CLIモデルレジストリ運用追加）と MCP CE-4 audit（8→9）を実チェック数へ同期した。
...
bash scripts/verify_business_flow_e2e.sh 8005   # 期待: "Result: 833 passed, 0 failed"（シナリオ1〜138）
```
...
| 成果物 | 役割 | 検証コマンド |
|--------|------|-------------|
| `03_Implement/backend/scripts/verify_business_flow_e2e.sh` | 業務フローE2E（シナリオ1〜138・833 checks） | §2.1 |
...
| `01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md` | シナリオ定義・カバレッジ集約（シナリオ138・チェック833） | §2.1 数値照合 |
