...
## 1. 現状サマリ（2026-08-17・iteration 240 時点）

| 指標 | 値 | 検証方法 |
|------|----|---------|
| 総シナリオ数（シナリオ1〜170） | 170 | §2.1 実走行＋`business-flow-e2e-scenarios-2026-08-15.md` |
| 業務フローE2Eスクリプトのシナリオヘッダ数 | 168 | verify script（シナリオ1=初期フロー・ヘッダechoなし / シナリオ4=別スクリプト`verify_admin_ops_flow_e2e.sh`） |
| カバー業態数 | 170 | シナリオドキュメント「カバレッジ集約」 |
| 業務フローE2Eチェック数 | 1026 | §2.1 実走行 → `Result: 1026 passed, 0 failed` |
| 総チェック数（check 10） | **1084** | = 1026（業務フロー）＋ 20（admin ops）＋ 7（kj multi-round）＋ 16（MCP CE-4 audit）＋ 15（CLI CE-4 audit）— 算術を verify script で照合 |
| 発行済み課題 | 22 Done（…DOGFOOD-30・DOGFOOD-31） | verify script |
| 検証層 | backend単体・frontend単体・frontend UI E2E・業務フローE2E・計画文書整合 | §2 |

> 注記: 総チェック数は**業務フローE2Eのチェック数（1026）とは別**。CIハーネス `verify_all.sh` check 10 は5自己完結E2E（業務フロー・admin ops・kj multi-round・MCP CE-4 audit・CLI CE-4 audit）を実行するため、総数 = 1084。以前の進捗報告で総数が誤記されていた差分（-10）は、本マニフェストの算術照合により検出・訂正された。iteration 195 の実走行照合で、並行編集者が拡張した admin ops（12→20・CLIモデルレジストリ運用追加）と MCP CE-4 audit（8→9）を実チェック数へ同期した。
...
bash scripts/verify_business_flow_e2e.sh 8005   # 期待: "Result: 1026 passed, 0 failed"（シナリオ1〜170）
```
...
| `01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md` | シナリオ定義・カバレッジ集約（シナリオ170・チェック1026） | §2.1 数値照合 |
...
| `issue-DOGFOOD-31-two-hundred-card-scale-exceeds-ai-operation-limits.md` | **Done** | 高度ドッグフーディング・第1ラウンド200枚（2026-08-17） | card-groups 100→1000 緩和 ＋ mock接地10件キャップ ＋ scenario 170 ③（200枚→10領域） |
...
| iteration 198 | シナリオ128（演劇・舞台芸術・創造性と経営の相克・矛盾検出の正パスを追加）→ 773/773 |
| iteration 199 | シナリオ129（酒類・醸造・品質と経営の相克・矛盾検出の正パスを追加）→ 779/779 |
| iteration 200 | シナリオ130（旅行代理店・丁寧さと効率の相克・矛盾検出の正パスを追加）→ 785/785 |
| iteration 201 | シナリオ131（クリーニング・品質と効率の相克・矛盾検出の正パスを追加）→ 791/791 |
...
| iteration 206 | シナリオ136（出版社・書籍出版・内容と採算の相克・矛盾検出の正パスを追加）→ 821/821 |
| iteration 207 | シナリオ137（医療機器・安全性とスピードの相克・矛盾検出の正パスを追加）→ 827/827 |
| iteration 208 | シナリオ138（石油・ガス・安定・保安と効率の相克・矛盾検出の正パスを追加）→ 833/833 |
| iteration 209 | シナリオ139（家電量販店・接客と価格の相克・矛盾検出の正パスを追加）→ 839/839 |
| iteration 210 | シナリオ140（宝飾・ジュエリー・品質と採算の相克・矛盾検出の正パスを追加）→ 845/845 |
| iteration 211 | シナリオ141（引越し・丁寧さと効率の相克・矛盾検出の正パスを追加）→ 851/851 |
| iteration 212 | シナリオ142（カメラ・写真・品質と効率の相克・矛盾検出の正パスを追加）→ 857/857 |
| iteration 213 | シナリオ143（医薬品卸・供給と効率の相克・矛盾検出の正パスを追加）→ 863/863 |
| iteration 214 | シナリオ144（スポーツ用品・専門性と価格の相克・矛盾検出の正パスを追加）→ 869/869 |
| iteration 215 | シナリオ145（コンサート・ライブ運営・音楽の質と集客の相克・矛盾検出の正パスを追加）→ 875/875 |
| iteration 216 | シナリオ146（神社・寺・伝統と運営の相克・矛盾検出の正パスを追加）→ 881/881 |
| iteration 217 | シナリオ147（学校給食・安全・栄養とコストの相克・矛盾検出の正パスを追加）→ 887/887 |
| iteration 218 | シナリオ148（陶磁器・工芸品・技法と効率の相克・矛盾検出の正パスを追加）→ 893/893 |
| iteration 219 | シナリオ149（牧場・酪農・福祉と採算の相克・矛盾検出の正パスを追加）→ 899/899 |
| iteration 220 | シナリオ150（墓地・霊園・供養と運営の相克・矛盾検出の正パスを追加）→ 905/905 |
| iteration 221 | シナリオ151（公営競技・公正性と収益の相克・矛盾検出の正パスを追加）→ 911/911 |
| iteration 222 | シナリオ152（税関・通関・正確性とスピードの相克・矛盾検出の正パスを追加）→ 917/917 |
| iteration 223 | シナリオ153（ガソリンスタンド・安全と価格の相克・矛盾検出の正パスを追加）→ 923/923 |
| iteration 224 | シナリオ154（カジノ・IR・健全性と収益の相克・矛盾検出の正パスを追加）→ 929/929 |
| iteration 225 | シナリオ155（ネイル・エステ・品質・衛生と効率の相克・矛盾検出の正パスを追加）→ 935/935 |
| iteration 226 | シナリオ156（ペンション・民宿・おもてなしと効率の相克・矛盾検出の正パスを追加）→ 941/941 |
| iteration 227 | シナリオ157（酒場・バー・質と集客の相克・矛盾検出の正パスを追加）→ 947/947 |
| iteration 228 | シナリオ158（音楽教室・教育の質と効率の相克・矛盾検出の正パスを追加）→ 953/953 |
| iteration 229 | シナリオ159（ペットケア・福祉と効率の相克・矛盾検出の正パスを追加）→ 959/959 |
| iteration 230 | シナリオ160（フードトラック・品質と出店の相克・矛盾検出の正パスを追加）→ 965/965 |
| iteration 231 | シナリオ161（介護・福祉用具・利用者本位と効率の相克・矛盾検出の正パスを追加）→ 971/971 |
| iteration 232 | シナリオ162（シェアオフィス・環境・コミュニティと稼働率の相克・矛盾検出の正パスを追加）→ 977/977 |
| iteration 233 | シナリオ163（ドローン事業・安全と効率の相克・矛盾検出の正パスを追加）→ 983/983 |
| iteration 234 | シナリオ164（パン屋・ベーカリー・こだわりと効率の相克・矛盾検出の正パスを追加）→ 989/989 |
| iteration 235 | シナリオ165（ゴルフ場・品質とコストの相克・矛盾検出の正パスを追加）→ 995/995 |
| iteration 236 | シナリオ166（漫画喫茶・ネットカフェ・環境・サービスと稼働の相克・矛盾検出の正パスを追加）→ 1001/1001 |
| iteration 237 | シナリオ167（眼鏡店・コンタクト・目の健康と効率の相克・矛盾検出の正パスを追加）→ 1007/1007 |
| iteration 238 | シナリオ168（建築設計事務所・デザインとコストの相克・矛盾検出の正パスを追加）→ 1013/1013 |
| iteration 239 | シナリオ169（クルーズ・観光船・安全・サービスと収益の相克・矛盾検出の正パスを追加）→ 1019/1019 |
