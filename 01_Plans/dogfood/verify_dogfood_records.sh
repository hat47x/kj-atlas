...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ165・チェック995）
if grep -q -e "シナリオ165・チェック995" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ165・チェック995)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ165・チェック995)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 1053 = 995 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=995
ADMIN_CHECKS=20
...
check "総チェック算術 (995+20+7+16+15)" "$TOTAL" "1053"
