...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ167・チェック1007）
if grep -q -e "シナリオ167・チェック1007" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ167・チェック1007)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ167・チェック1007)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 1065 = 1007 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=1007
ADMIN_CHECKS=20
MULTI_ROUND_CHECKS=7
MCP_CE4_CHECKS=16
CLI_CE4_CHECKS=15
TOTAL=$((BUSINESS_CHECKS + ADMIN_CHECKS + MULTI_ROUND_CHECKS + MCP_CE4_CHECKS + CLI_CE4_CHECKS))
check "総チェック算術 (1007+20+7+16+15)" "$TOTAL" "1065"
