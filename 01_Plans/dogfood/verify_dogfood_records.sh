...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ164・チェック989）
if grep -q -e "シナリオ164・チェック989" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ164・チェック989)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ164・チェック989)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 1047 = 989 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=989
ADMIN_CHECKS=20
...
check "総チェック算術 (989+20+7+16+15)" "$TOTAL" "1047"
