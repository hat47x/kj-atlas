...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ131・チェック791）
if grep -q -e "シナリオ131・チェック791" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ131・チェック791)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ131・チェック791)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 842 = 791 + 20 + 7 + 9 + 15）
BUSINESS_CHECKS=791
ADMIN_CHECKS=20
...
check "総チェック算術 (791+20+7+9+15)" "$TOTAL" "842"
