...
# 1. 業務フローE2Eスクリプトのシナリオヘッダ数（マニフェスト主張: 65）
#    シナリオ1=初期フロー（ヘッダechoなし）・シナリオ4=別スクリプトのため、
#    総シナリオ67 とは一致しない（マニフェスト §1 に注記）。
EXPECT_HEADERS=65
...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ67・チェック387）
if grep -q -e "シナリオ67・チェック387" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ67・チェック387)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ67・チェック387)"
  FAIL=$((FAIL + 1))
fi

# 3. シナリオドキュメントの状態リストにシナリオ67 が含まれる
if grep -q -e "シナリオ67（iteration 137" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "状態リストにシナリオ67 (iteration 137)" "ok" "ok"
else
  echo "  FAIL: 状態リストにシナリオ67 (iteration 137)"
  FAIL=$((FAIL + 1))
fi
