...
# 1. 業務フローE2Eスクリプトのシナリオヘッダ数（マニフェスト主張: 81）
#    シナリオ1=初期フロー（ヘッダechoなし）・シナリオ4=別スクリプトのため、
#    総シナリオ83 とは一致しない（マニフェスト §1 に注記）。
EXPECT_HEADERS=81
...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ83・チェック483）
if grep -q -e "シナリオ83・チェック483" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ83・チェック483)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ83・チェック483)"
  FAIL=$((FAIL + 1))
fi

# 3. シナリオドキュメントの状態リストにシナリオ83 が含まれる
if grep -q -e "シナリオ83（iteration 153" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "状態リストにシナリオ83 (iteration 153)" "ok" "ok"
else
  echo "  FAIL: 状態リストにシナリオ83 (iteration 153)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 525 = 483 + 12 + 7 + 8 + 15）
BUSINESS_CHECKS=483
ADMIN_CHECKS=12
...
check "総チェック算術 (483+12+7+8+15)" "$TOTAL" "525"
