...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ104・チェック609）
if grep -q -e "シナリオ104・チェック609" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ104・チェック609)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ104・チェック609)"
  FAIL=$((FAIL + 1))
fi

# 3. シナリオドキュメントの状態リストにシナリオ99 が含まれる
if grep -q -e "シナリオ104（iteration 174" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "状態リストにシナリオ104 (iteration 174)" "ok" "ok"
else
  echo "  FAIL: 状態リストにシナリオ104 (iteration 174)"
  FAIL=$((FAIL + 1))
fi
...
check "総チェック算術 (609+12+7+8+15)" "$TOTAL" "651"
