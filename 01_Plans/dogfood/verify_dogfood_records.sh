...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ135・チェック815）
if grep -q -e "シナリオ135・チェック815" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ135・チェック815)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ135・チェック815)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 868 = 815 + 20 + 7 + 11 + 15）
BUSINESS_CHECKS=815
ADMIN_CHECKS=20
...
check "総チェック算術 (815+20+7+11+15)" "$TOTAL" "868"
