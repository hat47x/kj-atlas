...
# 1. 業務フローE2Eスクリプトのシナリオヘッダ数（マニフェスト主張: 77）
#    シナリオ1=初期フロー（ヘッダechoなし）・シナリオ4=別スクリプトのため、
#    総シナリオ79 とは一致しない（マニフェスト §1 に注記）。
EXPECT_HEADERS=77
...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ79・チェック459）
if grep -q -e "シナリオ79・チェック459" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ79・チェック459)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ79・チェック459)"
  FAIL=$((FAIL + 1))
fi

# 3. シナリオドキュメントの状態リストにシナリオ79 が含まれる
if grep -q -e "シナリオ79（iteration 149" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "状態リストにシナリオ79 (iteration 149)" "ok" "ok"
else
  echo "  FAIL: 状態リストにシナリオ79 (iteration 149)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 501 = 459 + 12 + 7 + 8 + 15）
BUSINESS_CHECKS=459
ADMIN_CHECKS=12
...
check "総チェック算術 (459+12+7+8+15)" "$TOTAL" "501"
