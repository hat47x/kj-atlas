...
# 1. 業務フローE2Eスクリプトのシナリオヘッダ数（マニフェスト主張: 86）
#    シナリオ1=初期フロー（ヘッダechoなし）・シナリオ4=別スクリプトのため、
#    総シナリオ88 とは一致しない（マニフェスト §1 に注記）。
EXPECT_HEADERS=86
...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ88・チェック513）
if grep -q -e "シナリオ88・チェック513" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ88・チェック513)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ88・チェック513)"
  FAIL=$((FAIL + 1))
fi

# 3. シナリオドキュメントの状態リストにシナリオ88 が含まれる
if grep -q -e "シナリオ88（iteration 158" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "状態リストにシナリオ88 (iteration 158)" "ok" "ok"
else
  echo "  FAIL: 状態リストにシナリオ88 (iteration 158)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 555 = 513 + 12 + 7 + 8 + 15）
BUSINESS_CHECKS=513
ADMIN_CHECKS=12
...
check "総チェック算術 (513+12+7+8+15)" "$TOTAL" "555"
