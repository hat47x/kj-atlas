...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ128・チェック773）
if grep -q -e "シナリオ128・チェック773" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ128・チェック773)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ128・チェック773)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 824 = 773 + 20 + 7 + 9 + 15）
BUSINESS_CHECKS=773
ADMIN_CHECKS=20
...
check "総チェック算術 (773+20+7+9+15)" "$TOTAL" "824"
