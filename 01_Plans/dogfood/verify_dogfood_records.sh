...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ142・チェック857）
if grep -q -e "シナリオ142・チェック857" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ142・チェック857)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ142・チェック857)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 915 = 857 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=857
ADMIN_CHECKS=20
...
check "総チェック算術 (857+20+7+16+15)" "$TOTAL" "915"
