...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ129・チェック779）
if grep -q -e "シナリオ129・チェック779" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ129・チェック779)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ129・チェック779)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 830 = 779 + 20 + 7 + 9 + 15）
BUSINESS_CHECKS=779
ADMIN_CHECKS=20
...
check "総チェック算術 (779+20+7+9+15)" "$TOTAL" "830"
