...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ138・チェック833）
if grep -q -e "シナリオ138・チェック833" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ138・チェック833)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ138・チェック833)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 889 = 833 + 20 + 7 + 14 + 15）
BUSINESS_CHECKS=833
ADMIN_CHECKS=20
...
check "総チェック算術 (833+20+7+14+15)" "$TOTAL" "889"
