...
# 1. 業務フローE2Eスクリプトのシナリオヘッダ数（マニフェスト主張: 89）
#    シナリオ1=初期フロー（ヘッダechoなし）・シナリオ4=別スクリプトのため、
#    総シナリオ91 とは一致しない（マニフェスト §1 に注記）。
EXPECT_HEADERS=89
...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ91・チェック531）
if grep -q -e "シナリオ91・チェック531" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ91・チェック531)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ91・チェック531)"
  FAIL=$((FAIL + 1))
fi

# 3. シナリオドキュメントの状態リストにシナリオ91 が含まれる
if grep -q -e "シナリオ91（iteration 161" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "状態リストにシナリオ91 (iteration 161)" "ok" "ok"
else
  echo "  FAIL: 状態リストにシナリオ91 (iteration 161)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 573 = 531 + 12 + 7 + 8 + 15）
BUSINESS_CHECKS=531
ADMIN_CHECKS=12
...
check "総チェック算術 (531+12+7+8+15)" "$TOTAL" "573"
