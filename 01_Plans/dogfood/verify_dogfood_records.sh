...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ143・チェック863）
if grep -q -e "シナリオ143・チェック863" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ143・チェック863)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ143・チェック863)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 921 = 863 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=863
ADMIN_CHECKS=20
...
check "総チェック算術 (863+20+7+16+15)" "$TOTAL" "921"
