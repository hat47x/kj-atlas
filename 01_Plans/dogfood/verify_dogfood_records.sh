...
# 1. 業務フローE2Eスクリプトのシナリオヘッダ数（マニフェスト主張: 80）
#    シナリオ1=初期フロー（ヘッダechoなし）・シナリオ4=別スクリプトのため、
#    総シナリオ82 とは一致しない（マニフェスト §1 に注記）。
EXPECT_HEADERS=80
...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ82・チェック477）
if grep -q -e "シナリオ82・チェック477" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ82・チェック477)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ82・チェック477)"
  FAIL=$((FAIL + 1))
fi

# 3. シナリオドキュメントの状態リストにシナリオ82 が含まれる
if grep -q -e "シナリオ82（iteration 152" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "状態リストにシナリオ82 (iteration 152)" "ok" "ok"
else
  echo "  FAIL: 状態リストにシナリオ82 (iteration 152)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 519 = 477 + 12 + 7 + 8 + 15）
BUSINESS_CHECKS=477
ADMIN_CHECKS=12
...
check "総チェック算術 (477+12+7+8+15)" "$TOTAL" "519"
