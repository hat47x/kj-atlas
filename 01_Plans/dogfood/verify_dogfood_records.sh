...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ145・チェック875）
if grep -q -e "シナリオ145・チェック875" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ145・チェック875)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ145・チェック875)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 933 = 875 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=875
ADMIN_CHECKS=20
...
check "総チェック算術 (875+20+7+16+15)" "$TOTAL" "933"
