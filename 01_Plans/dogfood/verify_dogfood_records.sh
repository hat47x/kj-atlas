...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ101・チェック591）
if grep -q -e "シナリオ101・チェック591" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ101・チェック591)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ101・チェック591)"
  FAIL=$((FAIL + 1))
fi

# 3. シナリオドキュメントの状態リストにシナリオ99 が含まれる
if grep -q -e "シナリオ101（iteration 171" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "状態リストにシナリオ101 (iteration 171)" "ok" "ok"
else
  echo "  FAIL: 状態リストにシナリオ101 (iteration 171)"
  FAIL=$((FAIL + 1))
fi
...
check "総チェック算術 (591+12+7+8+15)" "$TOTAL" "633"
