...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ151・チェック911）
if grep -q -e "シナリオ151・チェック911" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ151・チェック911)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ151・チェック911)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 969 = 911 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=911
ADMIN_CHECKS=20
...
check "総チェック算術 (911+20+7+16+15)" "$TOTAL" "969"
