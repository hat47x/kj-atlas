...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ162・チェック977）
if grep -q -e "シナリオ162・チェック977" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ162・チェック977)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ162・チェック977)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 1035 = 977 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=977
ADMIN_CHECKS=20
...
check "総チェック算術 (977+20+7+16+15)" "$TOTAL" "1035"
