...
# 1. 業務フローE2Eスクリプトのシナリオヘッダ数（マニフェスト主張: 85）
#    シナリオ1=初期フロー（ヘッダechoなし）・シナリオ4=別スクリプトのため、
#    総シナリオ87 とは一致しない（マニフェスト §1 に注記）。
EXPECT_HEADERS=85
...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ87・チェック507）
if grep -q -e "シナリオ87・チェック507" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ87・チェック507)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ87・チェック507)"
  FAIL=$((FAIL + 1))
fi

# 3. シナリオドキュメントの状態リストにシナリオ87 が含まれる
if grep -q -e "シナリオ87（iteration 157" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "状態リストにシナリオ87 (iteration 157)" "ok" "ok"
else
  echo "  FAIL: 状態リストにシナリオ87 (iteration 157)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 549 = 507 + 12 + 7 + 8 + 15）
BUSINESS_CHECKS=507
ADMIN_CHECKS=12
...
check "総チェック算術 (507+12+7+8+15)" "$TOTAL" "549"
