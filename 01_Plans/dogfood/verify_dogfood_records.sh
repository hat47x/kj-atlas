...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ147・チェック887）
if grep -q -e "シナリオ147・チェック887" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ147・チェック887)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ147・チェック887)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 945 = 887 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=887
ADMIN_CHECKS=20
...
check "総チェック算術 (887+20+7+16+15)" "$TOTAL" "945"
