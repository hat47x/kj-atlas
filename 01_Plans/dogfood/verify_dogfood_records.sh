...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ126・チェック761）
if grep -q -e "シナリオ126・チェック761" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ126・チェック761)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ126・チェック761)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 812 = 761 + 20 + 7 + 9 + 15）
BUSINESS_CHECKS=761
ADMIN_CHECKS=20
...
check "総チェック算術 (761+20+7+9+15)" "$TOTAL" "812"
