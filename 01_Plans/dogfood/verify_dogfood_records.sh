...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ150・チェック905）
if grep -q -e "シナリオ150・チェック905" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ150・チェック905)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ150・チェック905)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 963 = 905 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=905
ADMIN_CHECKS=20
...
check "総チェック算術 (905+20+7+16+15)" "$TOTAL" "963"
