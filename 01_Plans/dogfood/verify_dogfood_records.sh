...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ130・チェック785）
if grep -q -e "シナリオ130・チェック785" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ130・チェック785)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ130・チェック785)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 836 = 785 + 20 + 7 + 9 + 15）
BUSINESS_CHECKS=785
ADMIN_CHECKS=20
...
check "総チェック算術 (785+20+7+9+15)" "$TOTAL" "836"
