...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ127・チェック767）
if grep -q -e "シナリオ127・チェック767" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ127・チェック767)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ127・チェック767)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 818 = 767 + 20 + 7 + 9 + 15）
BUSINESS_CHECKS=767
ADMIN_CHECKS=20
...
check "総チェック算術 (767+20+7+9+15)" "$TOTAL" "818"
