...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ132・チェック797）
if grep -q -e "シナリオ132・チェック797" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ132・チェック797)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ132・チェック797)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 848 = 797 + 20 + 7 + 9 + 15）
BUSINESS_CHECKS=797
ADMIN_CHECKS=20
...
check "総チェック算術 (797+20+7+9+15)" "$TOTAL" "848"
