...
# 1. 業務フローE2Eスクリプトのシナリオヘッダ数（マニフェスト主張: 90）
#    シナリオ1=初期フロー（ヘッダechoなし）・シナリオ4=別スクリプトのため、
#    総シナリオ92 とは一致しない（マニフェスト §1 に注記）。
EXPECT_HEADERS=90
...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ92・チェック537）
if grep -q -e "シナリオ92・チェック537" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ92・チェック537)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ92・チェック537)"
  FAIL=$((FAIL + 1))
fi

# 3. シナリオドキュメントの状態リストにシナリオ92 が含まれる
if grep -q -e "シナリオ92（iteration 162" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "状態リストにシナリオ92 (iteration 162)" "ok" "ok"
else
  echo "  FAIL: 状態リストにシナリオ92 (iteration 162)"
  FAIL=$((FAIL + 1))
fi
...
check "総チェック算術 (537+12+7+8+15)" "$TOTAL" "579"
