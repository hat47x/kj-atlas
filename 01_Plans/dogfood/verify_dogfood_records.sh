...
# 1. 業務フローE2Eスクリプトのシナリオヘッダ数（マニフェスト主張: 79）
#    シナリオ1=初期フロー（ヘッダechoなし）・シナリオ4=別スクリプトのため、
#    総シナリオ81 とは一致しない（マニフェスト §1 に注記）。
EXPECT_HEADERS=79
...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ81・チェック471）
if grep -q -e "シナリオ81・チェック471" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ81・チェック471)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ81・チェック471)"
  FAIL=$((FAIL + 1))
fi

# 3. シナリオドキュメントの状態リストにシナリオ81 が含まれる
if grep -q -e "シナリオ81（iteration 151" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "状態リストにシナリオ81 (iteration 151)" "ok" "ok"
else
  echo "  FAIL: 状態リストにシナリオ81 (iteration 151)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 513 = 471 + 12 + 7 + 8 + 15）
BUSINESS_CHECKS=471
ADMIN_CHECKS=12
...
check "総チェック算術 (471+12+7+8+15)" "$TOTAL" "513"
