...
# 1. 業務フローE2Eスクリプトのシナリオヘッダ数（マニフェスト主張: 78）
#    シナリオ1=初期フロー（ヘッダechoなし）・シナリオ4=別スクリプトのため、
#    総シナリオ80 とは一致しない（マニフェスト §1 に注記）。
EXPECT_HEADERS=78
...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ80・チェック465）
if grep -q -e "シナリオ80・チェック465" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ80・チェック465)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ80・チェック465)"
  FAIL=$((FAIL + 1))
fi

# 3. シナリオドキュメントの状態リストにシナリオ80 が含まれる
if grep -q -e "シナリオ80（iteration 150" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "状態リストにシナリオ80 (iteration 150)" "ok" "ok"
else
  echo "  FAIL: 状態リストにシナリオ80 (iteration 150)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 507 = 465 + 12 + 7 + 8 + 15）
BUSINESS_CHECKS=465
ADMIN_CHECKS=12
...
check "総チェック算術 (465+12+7+8+15)" "$TOTAL" "507"
