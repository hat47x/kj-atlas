...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ149・チェック899）
if grep -q -e "シナリオ149・チェック899" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ149・チェック899)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ149・チェック899)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 957 = 899 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=899
ADMIN_CHECKS=20
...
check "総チェック算術 (899+20+7+16+15)" "$TOTAL" "957"
