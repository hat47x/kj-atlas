...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ152・チェック917）
if grep -q -e "シナリオ152・チェック917" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ152・チェック917)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ152・チェック917)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 975 = 917 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=917
ADMIN_CHECKS=20
...
check "総チェック算術 (917+20+7+16+15)" "$TOTAL" "975"
