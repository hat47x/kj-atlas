...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ146・チェック881）
if grep -q -e "シナリオ146・チェック881" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ146・チェック881)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ146・チェック881)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 939 = 881 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=881
ADMIN_CHECKS=20
...
check "総チェック算術 (881+20+7+16+15)" "$TOTAL" "939"
