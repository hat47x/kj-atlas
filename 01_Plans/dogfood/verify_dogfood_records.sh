...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ139・チェック839）
if grep -q -e "シナリオ139・チェック839" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ139・チェック839)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ139・チェック839)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 895 = 839 + 20 + 7 + 14 + 15）
BUSINESS_CHECKS=839
ADMIN_CHECKS=20
...
check "総チェック算術 (839+20+7+14+15)" "$TOTAL" "895"
