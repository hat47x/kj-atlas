...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ144・チェック869）
if grep -q -e "シナリオ144・チェック869" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ144・チェック869)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ144・チェック869)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 927 = 869 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=869
ADMIN_CHECKS=20
...
check "総チェック算術 (869+20+7+16+15)" "$TOTAL" "927"
