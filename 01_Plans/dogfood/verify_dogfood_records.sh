...
# 2. シナリオドキュメントのカバレッジ集約ヘッダ（シナリオ148・チェック893）
if grep -q -e "シナリオ148・チェック893" "$ROOT/01_Plans/dogfood/business-flow-e2e-scenarios-2026-08-15.md"; then
  check "シナリオドキュメント カバレッジ集約 (シナリオ148・チェック893)" "ok" "ok"
else
  echo "  FAIL: シナリオドキュメント カバレッジ集約 (シナリオ148・チェック893)"
  FAIL=$((FAIL + 1))
fi
...
# 7. 総チェック数の算術照合（マニフェスト主張: 951 = 893 + 20 + 7 + 16 + 15）
BUSINESS_CHECKS=893
ADMIN_CHECKS=20
...
check "総チェック算術 (893+20+7+16+15)" "$TOTAL" "951"
