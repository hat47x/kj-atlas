...
| `doc_kj_atlas_dogfood_r5.json` | R5 具体策: 短期・中期・長期の3段階実行計画 | カード化完了 + 短期3策完了を反映 |
...
| 実API検証操作数 | `ai_eval_results.md` | 0/2（`run_ai_eval.py`で完全自動化、API key投入で実行） |
| 自律実行率 | 四半期判定ADR | L2: ①②達成+③自動化 / L3: 基盤整備+①達成 |

### コード生成成功率（L3自律基準①）

| 種別 | 成功率 | 正本 |
|------|--------|------|
| 骨格生成 | 80%（4/5） | `codegen_results.md` |
| ロジック生成 | 100%（3/3） | `codegen_results.md` |
...
| 2026-08-11 | 設計判断の三要素整合を人間の記憶に依存 | 全75 ADRに三要素検証を適用（72 Accepted + 3 Proposed暫定） |
