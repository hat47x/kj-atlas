...
第三者sessionの結果を見た後で、SUI Sensemakingに有利な評価基準や分類を作らないよう、分析の順序と判定単位を先に固定する。
...
「この人はSUI Sensemakingに向いている／向いていない」といった人物分類を、主要な分析単位にしない。
...
4. SUI Sensemakingの機能名ではなく、利用者の仕事として表現できる。
5. 使わない条件も対として説明できる。
