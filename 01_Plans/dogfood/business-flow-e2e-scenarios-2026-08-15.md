...
| 操作内容 | 文書作成 → **モデル一覧取得(available-models・R2データ源)** → **許容モデル明示選択で生成(generate-narrative+model)** → **テナント許容リスト設定(admin)** → 一覧が制限を反映 → **非許容モデルは403(model_not_allowed・R3 fail-closed)** |
| 注意事項 | モデル選択は**テナント許容リスト**に制限される（空=プラットフォーム既定=全許可）。非許容モデルは **LLM 呼び出し前に fail-closed(403)**・自動降格しない |

シナリオ47は **AI運用・ITガバナンス**という新業態で、モデル選択とテナント許容制限を固定。**AI-MODEL-GOVERNANCE-01 の R2 モデルセレクタデータ源（GET /ai/available-models）**と**R3 許容制限の fail-closed 適用（非許容モデル→403 model_not_allowed）**を業務フローE2Eとして初固定（管理APIでの許容リスト設定→一覧反映→遮断まで一連の運用フロー）。**259/259 pass**（シナリオ1〜47・47業態）。
