...
- **既に十分に決定・文書化済みと確認できたもの**（本ADRの対象外・再決定しない）:
  ...
  - エスカレーション方針・品質ゲート・HIL-RS A1契約（`llm_escalation_policy.md`・`llm_quality_strategy.md`・`hil_rs_01_a1_minimum_interface_contract.md`）は凍結済み。
...
- **ランタイム切替 UI は提供しない**: provider の変更は既存どおり運用者による環境変数設定＋再起動のみとする。これは SafeMode が既定ONで運用者制御のままである既存方針（`enterprise_architecture.md` §4）と同じガバナンス境界であり、エンドユーザーが個別に `large-scale`（外部送信）へ昇格できてしまう抜け道を作らない。
