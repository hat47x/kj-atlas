...
|------|----------------|---------------|
| **業務設計** | 利用者が「レビュー済み」状態と「誰が起票したか」主体を混同しないよう、両者を分離して扱う。主体メタデータ（起票者・作成者・所有者）の入力・編集・所有者移管・検索・管理者閲覧はMVP標準に含めない | 機能: 共有・export・レビューパック・外部エージェント依頼パッケージに主体メタデータを既定で含めない。データ: 主体情報が存在しない場合は推測せず「提供していない」と扱う |
| **データ設計** | カード本体は状態メタデータ（主張種別・保留・違和感・レビュー状態）だけを常設表示。主体メタデータは保存先・認証接続・権限・保持期間が未確定のため`Card.meta`を拡張しない | 業務: 個人・組織識別情報がimportや共有/exportの暗黙経路から保存・共有されるリスクを抑える。機能: 起票者追加はスキーマ・認証・権限・redactionをまとめた別判断が先 |
| **機能設計** | 詳細パネルで表示できるのは既存スキーマから導出できるカードID・代表/出典カード区分・文書作成/更新日時・`seq`/`source`に限る。import由来の未知キーを主体情報として保存しない | 業務: 組織内の責任追跡は現段階では完全には提供できないことを公開文書と製品化ゲートで明示。データ: 実装する場合は本ADRを更新し個人情報・監査・権限・保持・削除・共有範囲を別途確定 |
