...
|------|----------------|---------------|
| **業務設計** | Composeデプロイは公式PostgreSQL等のthird-partyイメージを使い、そのイメージは独自のコンテナ環境契約を持つ。「全環境変数が`SUI_*`」を「公開設定は`SUI_*`」と「デプロイ内の全プロセスが非KJ_ATLAS名を受け取らない」の2解釈に分けて明確な境界を定める | 機能: 非`SUI_*`変数は公開設定として文書化・受付・要求しない。データ: 利用者向け公開文書とrunbookは`SUI_*`のみを設定させる |
