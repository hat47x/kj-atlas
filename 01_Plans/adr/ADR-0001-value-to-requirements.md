...
- Data
  - `DATA-09-1`: 現在の可変 `DocumentV2` と、独立 `InquiryJourneyV1` が参照する不変 `RoundSnapshotV1` を分離する。
...
詳細要件は `00_Prompt/w_type_iterative_inquiry_requirements.md`、データ境界は `02_Architecture/inquiry_journey_model.md`、設計判断は `ADR-0057` を正本とする。
