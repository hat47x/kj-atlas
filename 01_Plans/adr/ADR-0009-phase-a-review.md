...
3. fail-safe
   - default無効（`llm.provider=none`）と `escalation.enabled=false` を4文書で一致。
   - `escalation.enabled=false` 時に external へフォールバックしない規則を固定。
4. 運用条件
...
| opt-in条件 | local/externalの有効化条件が文書ごとに曖昧 | localは明示opt-in、externalは`escalation.enabled=true`+allowlist-only outbound必須 |
...
- 4文書とも `llm.provider=none` を既定として扱う。
- 4文書とも external送信は opt-in + 制約付き（`escalation.enabled=true` と allowlist-only outbound）で一致。
