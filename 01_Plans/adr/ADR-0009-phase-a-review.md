...
- Date: 2026-03-02
- Scope: `02_Architecture/llm_provider_spec.md`, `llm_runtime_constraints.md`, `llm_quality_strategy.md`, `llm_escalation_policy.md`
...
3. fail-safe
   - default無効（`KJ_LLM_PROVIDER=none`）と `KJ_LLM_ESCALATION_ENABLED=false` を4文書で一致。
   - `KJ_LLM_ESCALATION_ENABLED=false` 時に external へフォールバックしない規則を固定。
4. 運用条件
...
| opt-in条件 | local/externalの有効化条件が文書ごとに曖昧 | localは明示opt-in、externalは`KJ_LLM_ESCALATION_ENABLED=true`+allowlist-only outbound必須 |
...
- 4文書とも `KJ_LLM_PROVIDER=none` を既定として扱う。
- 4文書とも external送信は opt-in + 制約付き（`KJ_LLM_ESCALATION_ENABLED=true` と allowlist-only outbound）で一致。
