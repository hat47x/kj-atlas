...
4. 共通の意味境界の下で、オフライン素材、外部素材、生成画像を段階導入する。実装は段階ごとに増えるが、利用者が負担とリスクに応じて選べる。
...
2. 供給経路を A: Unicode絵文字/同梱プリセット、B: 権利確認済み外部素材、C: proposal-only画像生成に分ける。Aから順に検証し、後の経路を前提にしない。
...
7. 採用済み手掛かりの保存先、画像バイナリの保持方式、容量上限、許容ライセンスallowlist、外部providerは本ADR受理後のfixture検証で決める。決定前に `DocumentV1` やDBへフィールド・テーブルを追加しない。
...
- [ ] 画像なし・絵文字・プリセット・外部素材・生成画像の代表タスク評価計画をUX Leadが確認する。
