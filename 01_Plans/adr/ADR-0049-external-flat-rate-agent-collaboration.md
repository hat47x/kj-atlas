...
- 移行時に必要な対応（Action は issue で管理）:
  - 仕様正本: `02_Architecture/external_agent_collaboration_spec.md`（本 ADR と同時制定）。
...
- Related: `02_Architecture/llm_provider_spec.md`, `02_Architecture/enterprise_architecture.md`（§4 アクセス制御/SafeMode/外部送出）, `02_Architecture/api.md`（context/export 監査）
