...
|------|----------------|---------------|
| **業務設計** | 従量課金APIはMVP段階でコスト障壁となる。企業・自治体の定額エージェント（Copilot等）に思考ワークロードを委ね、kj-atlasは文脈供給と結果受け入れ・レビューに徹する構成が現実解。人間が仲介する「依頼書書き出し→外部処理→応答取込→提案化」を標準ワークフローとする | 機能: 自動送信・自動適用を行わない。データ: 出るものはSafeMode適用・人間確認済みパッケージのみ |
