...
現行sui-sensemakingは、一つの文書内でカード化、グループ編成、図解、文章化、保留、違和感、根拠を扱える。しかし、6ラウンド累積KJ法のように、問題提起から手順化まで異なる姿勢でKJ法を繰り返し、現場での経験と思考を往復する長期的な探究は第一級の概念として持たない。
...
|------|----------------|---------------|
| **業務設計** | 長期的なKJ実務を行う利用者が問いと理解の変化を辿りながら現場取材を往復できるよう、探究を任意の高度機能として導入。人は各ラウンドの節目と引継ぎを確認し、AIは段階別の問い・引継ぎ・差分・反証候補を提案するだけで自動移行・過去成果の書換え・仮説決定はしない | 通常の一ラウンド利用と初期画面を変更せず、6段階の完了を強制しない。共有・移行時はmanifestと参照する全成果を自己完結bundleへ収め、SafeModeのマスクは元成果を書き換えない |
| **データ設計** | `InquiryJourneyV1`（manifest）がラウンドDAGを持ち、人は不変の`RoundSnapshotV1`を節目で残す。`RoundRecordV1`はstageとiterationを分離し、`RoundHandoffV1`が持ち越し・保留・未解決点を引き継ぐ。全体の唯一`activeRoundId`は持たず分岐先端を複数保持する | 探究は`DocumentV1`へ埋め込まず独立契約とし、現行`DocumentV1`のclosed-world validationと日常性能を巻き込まない。digestは破損確認用であり認可・秘匿・真正性の根拠にしない |
| **機能設計** | 探究は非破壊のラウンド成果DAG・分岐・自己完結bundleを提供し、前へ戻る操作は破壊的巻き戻しではなく「この成果から分岐」で表現する。詳細な型境界・保存・bundleは`inquiry_journey_model.html`を正本とする | 操作単位のイベントソーシングやCQRSを現段階で必須化しない。新しい型は実装・移行・運用CRUDが揃うまで`L0: Planned`。AIがラウンド移行・仮説採用・具体策決定を自動化しない |
...
- [x] 通常利用の初期表示を増やさず、高度機能へ置くことをUXプロトタイプで確認する。
- [x] `DOMAIN-W-ITERATION-01` の代表シナリオをマウス・キーボード・390pxで検証できる状態にする。
