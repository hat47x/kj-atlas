...
| FB-RM-MID-06 | 共通LLM adapter | P1 | Done (2026-03-01) | provider abstraction（none/local/large-scale）の共通I/Fを固定し、none既定・large-scale明示opt-in（`LLM_LARGE_SCALE_OPT_IN` + `LLM_ESCALATION_ENABLED`）・timeout/error/fallback契約を統一。監査最小フィールド（provider/model/transport/requested_at/trace_id/execution_path/fallback_to_none）を切替非依存で固定し、decision確定API非提供を回帰テストで検証。 | provider切替でUI/監査契約が変化せず、外部providerは明示opt-in時のみ利用される | `P-07`, `AI-07-*` |
