...
- sui-sensemaking はMVP期に、カード、島、レビュー、差分、AI提案、共有、診断を単一画面へ高密度に追加してきた。
...
- sui-sensemaking の製品化UIは、次の5つの画面領域または作業面を基本構造として扱う。
...
|------|----------------|---------------|
| **業務設計** | MVP期に単一画面へ高密度に追加した高度機能（レビュー・差分・AI提案・共有・診断）が、カード選択後に文脈に合う詳細を表示範囲へ出さない、共有操作が利用目的より実装順に近い等の摩擦を起こしている。初回利用者が「開く/作る・整理する・確認する・共有する」の流れを画面上で理解できる基本構造が必要 | 機能: カード/島選択直後は選択対象の確認・編集・レビューを現在の視野内へ出し高度機能は選択文脈の後ろへ。データ: 共有とエクスポートはファイル形式一覧でなく「何を誰と共有するか」起点の確認フローへ |
| **データ設計** | 5つの画面領域（開始/文書入口・キャンバス作業面・選択コンテキスト・作業モード面・共有前確認面）を基本構造として扱う。作業モード面はタブまたは明示モードとして段階的に開く | 業務: SafeModeと公開範囲の判断が共有直前に明示され誤共有リスクを下げる。機能: E2Eテストで画面領域ごとの期待操作を固定 |
| **機能設計** | 起動直後/初回は基本作業と安全状態を優先し、レガシー/高度/開発者寄り操作は補助導線。キーボード操作・狭い画面・大きな文書・低速環境は必須条件とし後付けの例外対応にしない | 業務: 全面リデザインの完成形は固定しない。データ: document/view/packスキーマ・レガシーimport/export・認証/認可方針は本ADRの範囲外 |
