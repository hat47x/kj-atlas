...
## 2026-08-27: `core.worktree`汚染が再発（今回は自分自身のworktree IDを指していた）。既存remedyで復旧、GIT_DIR越しの`docs_check.py`一括実行は避けて個別実行に切替

- 事象: AI-MODEL-GOVERNANCE-03のbackend検証中、Windows Bashツール側の`git diff --stat`が`fatal: Invalid path '/mnt': No such file or directory`で失敗した。セッション冒頭の`git checkout -b`は成功していたため、途中で発生した。
- 原因: 共有`.git/config`の`[core]`に`worktree = /mnt/c/GIT/kj-atlas/.claude/worktrees/agent-a80436a1597318c80`（**自分自身のworktree ID**）が書き込まれていた。2026-08-22/08-26ログと同一症状だが、汚染元が他worktreeではなく自分自身だった点が新しい。`docs_contract_checks.py`は`git ls-files`のみでconfig書き込みを行わないため、この特定セッションの操作が直接の書き込み元とは断定できなかった（原因未特定のまま、既知remedyで復旧した）。
- 対応: 2026-08-22/08-26と同じ手順（Bashツールの`sed -i`で該当1行のみ除去。Editツールは共有`.git/config`への書き込みを拒否するため使えない）で復旧し、Windows側`git status`で確認した。
- 追加の知見: `01_Plans/docs_check.py`を`GIT_DIR`/`GIT_WORK_TREE`環境変数越しにWSLから一括実行すると、2026-08-22ログの「誤った対処」がそのまま再現し（`_run_contract_tests`が`subprocess.run([sys.executable, "-m", "unittest", ...])`で子プロセスへ環境変数を継承させ、`01_Plans/tests/test_docs_contract_checks.py`内の一時repoフィクスチャが`-C`指定を無視して本worktreeを操作してしまう）、無関係な5件（2 FAIL + 3 ERROR）が発生した。**より簡潔な回避策**: `docs_check.py`の実チェックだけを`run_docs_check(root, run_tests=False)`を直接pythonから呼んで検証し（`_run_contract_tests`をスキップ）、`01_Plans/tests`・`01_Plans/issues/tests`の埋め込みunittestスイートは`GIT_DIR`/`GIT_WORK_TREE`を設定**しない**別呼び出しで実行する（これらのスイートは自前の一時repoで完結するテストが大半なので、実worktreeへのgit access自体は不要）。この2分割だけで済み、`.git`ポインタファイルの書き換え/書き戻し（2026-08-22の代替手順）や`rsync`での一時repo作成は不要だった。
- 再発防止: `Invalid path '/mnt'`が出たら、まず共有`.git/config`の`core.worktree`行を`sed -i`で除去する（Editツールは拒否されるためBash必須）。WSLから`01_Plans/docs_check.py`を動かす場合は、`GIT_DIR`/`GIT_WORK_TREE`をエクスポートしたシェルで丸ごと実行せず、(1) `run_docs_check(root, run_tests=False)`を直接呼ぶ検証と (2) 埋め込みunittestスイートの個別実行（環境変数なし）に分けること。
