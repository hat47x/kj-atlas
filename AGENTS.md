...
4) **W型反復的探究要件**: `00_Prompt/w_type_iterative_inquiry_requirements.md`
5) **申し送り（重複排除済）**: `00_Prompt/handoff.md`
6) **AI開発引き継ぎ運用**: `00_Prompt/agent_handover.md`
7) **Codex+GSD運用ルール**: `00_Prompt/codex_gsd_skill_ops.md`
8) **Codex+RTK token saving運用**: `00_Prompt/codex_rtk_token_saving_ops.md`
9) **認知外在化AI要件（新規中核）**: `00_Prompt/ai_cognitive_externalization_requirements.md`
10) **価値→要件**: `01_Plans/adr/ADR-0001-value-to-requirements.md`
11) **全体アーキテクチャ**: `02_Architecture/architecture.md`
...
- `00_Prompt/w_type_iterative_inquiry_requirements.md`：6ラウンド累積KJ法の反復・引継ぎ・停止再開・分岐を扱うNormative要件。実装は `DOMAIN-W-ITERATION-01` で段階導入する。
...
- `02_Architecture/history/README.md`：architecture/API/schema/data operationsのfreeze/Stream/checkpoint等を現行契約から分離したInformative形成履歴の収録規律と索引。
- `02_Architecture/schemas.md`：現行データスキーマ（document/view/pack等）の正本。形成履歴は `02_Architecture/history/README.md` から参照する。
- `02_Architecture/data_model_operations_overview.md`：MVPデータモデル、論理ER、CRUDサポート表、ステークホルダー別運用境界。
- `02_Architecture/inquiry_journey_model.md`：反復的探究の独立集約、不変成果DAG、カード系譜、自己完結bundle、段階導入の採択済み設計目標（現状L0）。
