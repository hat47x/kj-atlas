...
- **主体を解決できなかったリクエストにはこのフィールドが出ません。** `/healthz` のような未認証エンドポイントや、single-tenant プロファイルで認証ヘッダーが無いリクエストがこれに該当します。フィールドが無いことは異常ではなく、既定の「匿名」状態です。
- したがって、**エラーの主体そのもの（誰か）を特定する手段はログにはまだありません。** できるのは「同じ actor による複数のログ行を束ねる」ことまでです。個人を特定するにはDB側の追加調査（例: control-plane キーのローテーション履行記録、SaaS 側のユーザー管理画面）が必要です。監査イベント側の `actorRefHash` も同じ制約を持ちます。監査の宛先設定と保存については下記の制約を参照してください。
