...
`詳細` を ON にすると、レビュー差分・ナラティブ・マージ候補・パッチ・AI 提案などの高度機能が現れます。**現状これらは独立した領域ではなく選択コンテキスト（領域3）の内部に展開されます**（課題 `UX-NAV-01`。下記ブリーフ参照）。
...
- **`UX-NAV-01`（高）**: 作業モード面（領域4）に独立した領域が無く、高度機能が選択コンテキスト（領域3）内へ積層する（本書 §7 の画像が該当）。領域4へ独立領域（既定OFFのドロワー/オーバーレイ）を与え、選択直後に選択対象を前景化する。
