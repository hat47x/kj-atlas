...
- **CB-3 是正（2026-07-04）**: 右側パネルの詳細ドメインフィルタと Guided Flow は `詳細` ON 時だけ表示する。既定表示では、選択対象・状態サマリ・基本編集を優先する。
- **CB-3 残課題（中）**: 新しい操作を追加する場合は、既定表示への純増ではなく、置換・包含・モード分離で扱う（`UX-COMPLEXITY-01` の継続監査対象）。
