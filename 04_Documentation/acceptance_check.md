...
3. 右側パネルの先頭に「現在の選択」が表示され、選択したカード名、レビュー状態、次の操作が分かることを確認する。
4. 必要に応じて島を作成し、右側パネルの情報が更新されることを確認する。
5. 島を選択し、「現在の選択」がカードから島に切り替わることを確認する。
6. `表示` を開き、表示モードや SafeMode の状態を確認する。
7. `共有と再現` を開き、共有や書き出しの前に出力範囲と SafeMode を確認する。
8. 保存し、ページを再読み込みして変更が残っていることを確認する。
