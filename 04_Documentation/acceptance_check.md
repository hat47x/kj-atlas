...
起動後は「文書を開く・作成する」パネルが表示されることを確認します。新しい文書、サンプル、文書ファイル、レビューパックの入口が分かれ、SafeMode の状態が同じパネル内で確認できることを見ます。

![文書入口パネル](assets/screenshots/start-panel-document-entry.png)

## 画面全体を見る

開始パネルを閉じるか、サンプルを選んだあと、ヘッダー、キャンバス、右側パネルが見えることを確認します。ヘッダーには SafeMode、表示、検索、共有と再現、保存などの主要操作があります。
...
1. 新規ドキュメントを作成する。
2. サンプルを開き、画面上のカードと島を確認する。
3. 文書ファイルまたはレビューパックを読み込む場合は、取り込み前の検証結果を確認してから進める。
4. カードを選択し、キャンバス上で移動する。
5. 必要に応じて島を作成し、右側パネルの情報が更新されることを確認する。
...
1. `Tab` で開始パネルの新規作成、サンプル、文書ファイル、レビューパックの入口へ移動できることを確認する。
2. 開始パネルを閉じたあと、`Tab` でヘッダー、キャンバス操作、右側パネルへ移動できることを確認する。
3. フォーカス中のボタンを `Enter` または `Space` で実行できることを確認する。
4. `表示` や `共有と再現` のパネルを開いたあと、次に操作すべき項目へ自然に移動できることを確認する。
5. 文字入力欄では、入力、削除、保存または反映の流れが分かることを確認する。
6. 操作をやめたいときに、キャンセル、閉じる、または別の場所への移動で自然に戻れることを確認する。
