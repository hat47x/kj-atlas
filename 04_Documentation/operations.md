...
画面側の入口は次の状態を目安にします。起動直後に「文書を開く・作成する」パネルが表示され、SafeMode が ON であること、新規作成、サンプル、文書ファイル、レビューパックの入口が選べることを確認します。ヘッダーに SafeMode、表示モード、共有と再現、保存などの主要操作があり、キャンバスと右側パネルが表示されていれば、次に API と保存の確認へ進めます。

![運用確認で見る文書入口](assets/screenshots/start-panel-document-entry.png)
