...
身近な例で言えば、同じ名前を全角・半角や余分な空白の違いで別物として扱わないようにする考え方です。kj-atlas では、データの保存、import/export、AI 提案の根拠確認で同じ考え方を使います。

## 前提知識

この文書を読むために、hash や schema の詳細を理解している必要はありません。まずは「同じ内容なら同じ形にそろえて比較する」という考え方だけ押さえれば十分です。

| 用語 | ここでの意味 |
| --- | --- |
| 正規化 | 表記ゆれや順序の違いをそろえること |
| hash | 内容が同じかを確認するための短い識別値 |
| 一時情報 | 生成時刻や trace id など、内容そのものではない情報 |
...
canonicalization は「比較を安定させる道具」であり、「内容が正しいと保証する仕組み」ではありません。内容の妥当性は、人間のレビューや E2E 確認で判断します。
