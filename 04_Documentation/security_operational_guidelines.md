...
読後にできること: 外部サービスとの共有、SafeMode、LLM、audit の設定を変える前に、進めてよい条件と止める条件を説明できます。

## 役割

| 役割 | 主な責務 |
| --- | --- |
| Security Officer | セキュリティ例外、外部サービスとの共有、SafeMode 緩和の可否を確認する |
...
| 用語 | 意味 |
| --- | --- |
| 進行条件（Go 条件） | 変更を進めてよい最低条件 |
| 停止条件（Stop 条件） | 1つでも当てはまれば変更を止める条件 |
| 保留 | すぐに判断せず、追加確認まで設定を変えない状態 |
| rollback | 変更前の安全な状態へ戻す手順 |

## この文書の使い方

セキュリティ設定を変える前に、該当する判断例を読み、進行条件と停止条件を確認します。すぐに判断できない場合は、設定を変えずに保留します。安全側に倒すことを優先してください。

## 変更前に確認する4点

1. 何を変えるか: 対象の環境変数、サービス、データ出力。
2. なぜ必要か: 利用者価値、障害対応、検証目的。
3. どの情報を誰・どのサービスと共有するか: LLM 入力、audit event、export、ログ。
...
- 宛先が local または組織内の接続先（endpoint）である。
- 接続先（endpoint）が `/generate` に `{ "text": "..." }` を返す。
- 入力に秘密情報を含めない運用が確認済み。
...
### 監査ログの HTTP 連携（audit HTTP export）を有効にする

進行条件:

- 監査ログ連携の接続先が管理済み。
- 待ち時間上限（timeout）と queue size が設定済み。
- 監査ログに含める payload に秘密情報が含まれない。

停止条件:

- 監査ログ連携の接続先の所有者や保持期間が不明。
- SafeMode 中の外部サービスとの共有を許可する理由が記録されていない。

どの判断例にも完全には当てはまらない場合は、設定変更を保留し、変更内容、想定される外部サービスとの共有、戻し方を短く記録してから確認者に回してください。
