      ...
      postMessage(message: { type: string; requestId: string }) {
        ...
        for (const [index, stage] of stages.entries()) {
          for (const listener of this.listeners) {
            listener({ data: { type: "diff.progress", requestId: message.requestId, stage, percent: (index + 1) * 20 } } as MessageEvent);
          }
        }
        for (const listener of this.listeners) {
          listener({ data: { type: "diff.result", requestId: message.requestId, result: { documentDiff: [], viewDiff: [] } } } as MessageEvent);
        }
      }
      terminate() {}
    }
