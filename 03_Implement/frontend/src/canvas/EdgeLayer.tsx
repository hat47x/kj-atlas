...
function EdgeLayerComponent({
  ...
  const drawableEdges = useMemo(() => {
    return edges.filter((edge) => {
      ...
      const fromCenter =
        edge.fromKind === "card" ? cardCenterById.get(edge.fromId) : islandCenterById.get(edge.fromId);
      const toCenter =
        edge.toKind === "card" ? cardCenterById.get(edge.toId) : islandCenterById.get(edge.toId);
  ...
  return (
    ...
    >
      {drawableEdges.map((edge) => {
        ...
        const stroke = isSelected ? "#2563eb" : edge.isDerived ? "#0f766e" : "#64748b";
        const strokeWidth = isSelected ? 3 : edge.isDerived ? 2.5 : 2;
