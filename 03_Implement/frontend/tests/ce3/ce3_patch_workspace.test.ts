...
} from "../../src/domain/patch/workspace/ce3_patch_workspace";

describe("ce3_patch_workspace (Stream D subtree)", () => {
  it("supports hold/adopt/reject transitions and rollback", () => {
    ...
    expect(rolledBack.auditLog.at(-1)).toMatchObject({ reason: "rollback", candidateId: "cand-2" });
  });

  it("normalizes preset query deterministically", () => {
    expect(normalizeFilters(" b, A,  ,c ")).toEqual(["a", "b", "c"]);
    expect(normalizePresetQuery({ scope: "selection", depth: 2.8, filters: ["z", "a"] })).toBe(
      '{"scope":"selection","depth":2,"filters":["a","z"]}'
    );
  });
  ...
  it("summarizes patch preview token delta for CE3 diff preview", () => {
    ...
    expect(summary).toEqual({ additions: 2, removals: 3, hasChanges: true });
  });
});
