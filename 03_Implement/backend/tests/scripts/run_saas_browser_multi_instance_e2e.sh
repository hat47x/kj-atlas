...
LOG_DIR="${RUNNER_TEMP:-/tmp}/sui-sensemaking-saas-browser-e2e"
mkdir -p "${LOG_DIR}"

: "${SUI_DATABASE_URL:?SUI_DATABASE_URL is required}"
...
export SUI_ADMIN_API_KEY=e2e-admin-key
...
export SUI_ACCESS_CONTROL_EXTERNAL_HTTP_ENDPOINT="${POLICY_BASE_URL}/access-control"
export SUI_DOCUMENT_POLICY_BINDING_RESOLVER=external_http
export SUI_DOCUMENT_POLICY_BINDING_HTTP_ENDPOINT="${POLICY_BASE_URL}/document-policy"
export SUI_TENANT_CAPABILITY_RESOLVER=external_http
export SUI_TENANT_CAPABILITY_HTTP_ENDPOINT="${POLICY_BASE_URL}/capabilities"
export SUI_SAAS_OAUTH_BROKER_HTTP_AUTHORIZE_ENDPOINT="${BROKER_BROWSER_BASE_URL}/oauth/authorize"
export SUI_SAAS_OAUTH_BROKER_HTTP_TOKEN_ENDPOINT="${BROKER_BACKCHANNEL_BASE_URL}/oauth/token"
export SUI_SAAS_OAUTH_BROKER_HTTP_REDIRECT_URI="${BROWSER_BASE_URL}/session/callback"
export SUI_SAAS_OAUTH_BROKER_HTTP_CLIENT_ID=mock-client
export SUI_SAAS_OAUTH_BROKER_HTTP_CLIENT_SECRET=e2e-secret
export SUI_SAAS_AUTH_SESSION_HASH_KEY=0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef
export SUI_JWT_ALGORITHMS=RS256
export SUI_TENANT_CLAIM_NAME=tenant_ref
export SUI_LLM_PROVIDER=none
export SUI_MOCK_IDP_BASE="${BROKER_BROWSER_BASE_URL}"
export SUI_FRONTEND_API_BASE=/api
export SUI_E2E_WORKER_1=http://127.0.0.1:8001
export SUI_E2E_WORKER_2=http://127.0.0.1:8002
...
python -m uvicorn sui_sensemaking_api.main:app --host 127.0.0.1 --port 8001 >"${LOG_DIR}/worker1.log" 2>&1 & pids+=("$!")
python -m uvicorn sui_sensemaking_api.main:app --host 127.0.0.1 --port 8002 >"${LOG_DIR}/worker2.log" 2>&1 & pids+=("$!")
...
env -i \
  ...
  HOME="${HOME:-/tmp}" \
  CI="${CI:-}" \
  SUI_RUNTIME_PROFILE="${SUI_RUNTIME_PROFILE}" \
  SUI_FRONTEND_API_BASE="${SUI_FRONTEND_API_BASE}" \
