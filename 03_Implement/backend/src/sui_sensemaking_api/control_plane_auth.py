...
1. The provisioning surface had no authorization distinct from the business
   ...
   passes through when `SUI_API_KEY` is unset and otherwise accepts the same
...
- **Stage A — control-plane bearer.** `SUI_ADMIN_API_KEY` presented as
...
from sui_sensemaking_api.observability import bind_actor_ref_hash
...
if TYPE_CHECKING:
    from sui_sensemaking_api.saas_request_context import TrustedSaasRequestSession
...
#: production profiles without `SUI_ADMIN_API_KEY` (D3=A), so a production
#: runtime can never reach the open branch.
_OPEN_WHEN_UNCONFIGURED_PROFILES = frozenset({"local-dev", "evaluation"})

_CONTROL_PLANE_SUBJECT_STATE = "sui_sensemaking_control_plane_subject"
