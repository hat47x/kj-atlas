...
from sui_sensemaking_api.audit import CE4_AUDIT_SCHEMA_VERSION, normalize_ce4_audit_metadata


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="sui_sensemaking_api.cli")
    ...
    admin_parser = subparsers.add_parser(
        "admin",
        help="Operate the control plane using SUI_ADMIN_API_KEY.",
    )
...
def _business_plane_headers(*, actor_ref: str | None = None, trace_id: str | None = None) -> dict[str, str]:
    ...
    api_key = (os.environ.get("SUI_API_KEY") or "").strip()
    if api_key:
...
def _control_plane_headers(
    *, actor_ref: str | None = None, trace_id: str | None = None
    ...
    headers: dict[str, str] = {}
    admin_api_key = (os.environ.get("SUI_ADMIN_API_KEY") or "").strip()
    if admin_api_key:
