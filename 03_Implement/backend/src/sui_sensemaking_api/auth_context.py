...
from sui_sensemaking_api.identity_binding import (
    IdentityMappingConflictError,
    ensure_legacy_identity_provider,
    ensure_user_identity_binding,
    resolve_user_identity,
)
from sui_sensemaking_api.models import UserIdentityRow, UserRow
...
from sui_sensemaking_api.tenant_foundation import ensure_local_default_membership
...
def _check_trusted_proxy(request: Request) -> None:
    ...
    SUI_TRUSTED_PROXIES is configured, requests from non-proxy IPs
    ...
    if not raw_cidrs:
        if not _trusted_proxies_warned:
            ...
            logger.warning(
                ...
                "origin. Set SUI_TRUSTED_PROXIES to a comma-separated "
