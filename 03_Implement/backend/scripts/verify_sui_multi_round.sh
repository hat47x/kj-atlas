...
#
# LLM 切替（環境変数）:
#   SUI_DEEPSEEK_API_KEY 設定時 -> 実API（DeepSeek・課金）
#   未設定時                    -> ローカルモック（GPU不要・無料・決定的・iteration 41/42 の縮退方式）
#
# Usage:
#   bash verify_sui_multi_round.sh [PORT]
#   SUI_DEEPSEEK_API_KEY=<key> bash verify_sui_multi_round.sh [PORT]   # 実API
...
if [ -n "${SUI_DEEPSEEK_API_KEY:-}" ]; then
  LLM_PROVIDER=deepseek
  echo "  LLM: DeepSeek 実API（課金）"
else
  LLM_PROVIDER=local
  echo "  LLM: ローカルモック（無料・決定的）— SUI_DEEPSEEK_API_KEY 未設定"
fi
...
echo "=== sui-sensemaking 人間×AI 多ラウンド協調KJ検証 ==="
...
if [ "$LLM_PROVIDER" = "local" ]; then
  ...
  LOCAL_BASE_URL="${SUI_LOCAL_LLM_BASE_URL:-}"
fi
...
SUI_LLM_PROVIDER="$LLM_PROVIDER" \
SUI_LOCAL_LLM_BASE_URL="$LOCAL_BASE_URL" \
