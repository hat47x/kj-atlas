...
- trusted SaaS identity resolver、tenant resolver、active tenant session persisterはapplication起動前の単一bundleとしてのみ注入し、部分設定・起動後差し替え・不正bundleを拒否する。bundle非注入時はidentity/persisterをunavailable、tenant resolverをsingle-tenant互換に戻し、session APIをfail-closedに保つ
...
tenant/identity/membership、Document複合key、TenantContext/PDP/audit伝播、PostgreSQL RLS migrationとtransaction-local DB context、fail-closed bootstrap policy/session GET/active tenant POST route、trusted auth adapterの原子的な起動前bundle、profile別frontend entry、任意注入tenant switch App host、strict capability adapter、trusted outbound HTTPのredirect拒否、外部PDP request/responseとLLM応答のclosed-world・上限検証、外部PDP・監査HTTPの設定guardまでは実装済みである。session/capability境界ではresolverのmembership IDをDBのactive membershipから再生成した値と照合し、停止・差し替え証跡をPDP呼出し前に拒否する。ただしPostgreSQL実地検証、auth edgeからverified evidenceを生成する実resolverとanti-forgery付きsession persisterへの接続、`tenantSessionVersion`と全route precondition、production entryからApp hostへのsession注入、MCP/worker/cache/browser/storageを含む完全な越境matrixは未完了である。このため共有DB型のマルチテナントSaaSとして運用してはならず、`enterprise-production`は単一組織デプロイ向けと解釈する。`saas-multitenant`の起動拒否は残りの`SAAS-TENANT-01`条件を完了するまで維持する。
