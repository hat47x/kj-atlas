...
# --- External-agent contract: do not infer a merge method. ---
replace_text(
...
# The exported task sheet must request the same explicit method field.
replace_text(
...
# App.tsx is CRLF-heavy. Preserve bytes and only touch the imported merge-candidate block.
replace_bytes(
...
# --- New-decision producers must state the method. ---
replace_text(
...
# Deterministic fixtures are near-duplicate paths; say so explicitly.
replace_text(
...
# Some tests construct persisted decisions directly; method is optional there for old-data compatibility.
replace_text(
    "03_Implement/frontend/src/domain/merge_suggestion_apply.test.ts",
    '      snapshotVersion: "v2",\n      cardIds:',
    '      snapshotVersion: "v2",\n      mergeMethod: "near_duplicate",\n      cardIds:',
...
# UI fixture corresponds to deterministic normalized-text candidates.
replace_bytes(
...
# --- Persisted DocumentV1 contract: optional for legacy read, validated when present. ---
# types.ts is also CRLF-heavy.
replace_bytes(
...
# Strict validator regression: known values persist, legacy remains valid, unknown fails.
replace_text(
...
# Lenient import regression: preserve known method, do not invent one for old data.
replace_text(
  ...
  it("preserves a known mergeMethod and does not infer one for legacy decisions", () => {
    const result = validateImportedDocument({
      ...
      mergeSuggestionDecisions: [
        {
          id: "d1", groupId: "g1", decision: "accept", decidedAt: "2026-09-04T00:01:00.000Z",
          cardIds: ["c1", "c2"], mergedTextDraft: "A", editedText: "A", mergeMethod: "near_duplicate",
        },
        {
          id: "d2", groupId: "g2", decision: "defer", decidedAt: "2026-09-04T00:02:00.000Z",
...
# --- External-agent regressions and self-describing task sheet. ---
replace_text(
    ...
    for (const mergeMethod of ["near_duplicate", "kernel_fusion"] as const) {
      const proposal = {
        proposalId: `merge-${mergeMethod}`,
        kind: "merge_candidate",
        targetRef: { cardIds: ["c1", "c2"] },
        content: { mergedText: "統合候補", mergeMethod },
        rationale: "統合方式を明示する",
      };
  ...
  it("does not guess a missing or unknown mergeMethod for merge_candidate proposals", () => {
    for (const content of [
      { mergedText: "統合候補" },
      { mergedText: "統合候補", mergeMethod: "unknown_method" },
    ]) {
      const proposal = {
        proposalId: "merge-invalid",
        kind: "merge_candidate",
        targetRef: { cardIds: ["c1", "c2"] },
        content,
        rationale: "方式が不正",
...
# Golden task sheet contains the global response contract, so its schema gains the optional-by-kind field too.
replace_text(
...
# External-agent architecture spec: explicit method for merge candidates, never guessed.
replace_text(
