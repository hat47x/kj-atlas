...
TARGETS = (
    "issue-OPS-LLM-COST-03-view-panel-stale-after-ai-call.md",
    "issue-UX-TITLE-FOCUS-01-title-edit-tab-lost-focus.md",
)
ISSUES_ROOT = Path("01_Plans/issues")
DONE_ROOT = ISSUES_ROOT / "done"
VALIDATOR = ISSUES_ROOT / "validate_active_issue_memos.py"
SCRIPT_PATH = Path(".github/scripts/lane_c_ratchet_ui_operability_legacy_done_once.py")
...
def main() -> None:
    ...
    before = "LEGACY_DONE_AT_ROOT_BASELINE = 44"
    after = "LEGACY_DONE_AT_ROOT_BASELINE = 42"
    if validator_text.count(before) != 1:
        raise SystemExit("legacy Done baseline 44 の定義を一意に特定できません")
