...
TARGETS = (
    "issue-DX-CI-PG-02-alembic-postgresql-downgrade-untested.md",
    "issue-MCP-DOGFOOD-12-wsl-verifier-runtime-assumptions.md",
)
ISSUES_ROOT = Path("01_Plans/issues")
DONE_ROOT = ISSUES_ROOT / "done"
VALIDATOR = ISSUES_ROOT / "validate_active_issue_memos.py"
SCRIPT_PATH = Path(".github/scripts/lane_c_ratchet_verification_infra_legacy_done_once.py")
...
def main() -> None:
    ...
    before = "LEGACY_DONE_AT_ROOT_BASELINE = 41"
    after = "LEGACY_DONE_AT_ROOT_BASELINE = 39"
    if validator_text.count(before) != 1:
        raise SystemExit("legacy Done baseline 41 の定義を一意に特定できません")
