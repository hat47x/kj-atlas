...
TARGETS = (
    "issue-AI-MODEL-UX-02-stale-model-selection-after-admin-change.md",
    "issue-AI-TITLE-SAFEMODE-01-title-suggestion-review-contract.md",
)
ISSUES_ROOT = Path("01_Plans/issues")
DONE_ROOT = ISSUES_ROOT / "done"
VALIDATOR = ISSUES_ROOT / "validate_active_issue_memos.py"
SCRIPT_PATH = Path(".github/scripts/lane_c_ratchet_ai_ux_legacy_done_once.py")
...
def main() -> None:
    ...
    before = "LEGACY_DONE_AT_ROOT_BASELINE = 52"
    after = "LEGACY_DONE_AT_ROOT_BASELINE = 50"
    if validator_text.count(before) != 1:
        raise SystemExit("legacy Done baseline 52 の定義を一意に特定できません")
