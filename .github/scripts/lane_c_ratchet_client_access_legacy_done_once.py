...
TARGETS = (
    "issue-SEC-CLI-AUTH-01-ce4-cli-could-not-authenticate-to-keyed-backend.md",
    "issue-SEC-MCP-SCOPE-01-http-mcp-did-not-enforce-read-scope.md",
)
ISSUES_ROOT = Path("01_Plans/issues")
DONE_ROOT = ISSUES_ROOT / "done"
VALIDATOR = ISSUES_ROOT / "validate_active_issue_memos.py"
SCRIPT_PATH = Path(".github/scripts/lane_c_ratchet_client_access_legacy_done_once.py")
...
def main() -> None:
    ...
    before = "LEGACY_DONE_AT_ROOT_BASELINE = 46"
    after = "LEGACY_DONE_AT_ROOT_BASELINE = 44"
    if validator_text.count(before) != 1:
        raise SystemExit("legacy Done baseline 46 の定義を一意に特定できません")
